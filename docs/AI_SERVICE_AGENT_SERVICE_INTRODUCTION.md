...
IntelliDecision은 발화에 라벨 하나를 붙이는 분류기가 아니다. 이는 분류된 유형을 출발점으로 사용자의 **목적**, **이전 대화**, **필요한 정보**, **실행 위험도**, **다음 행동**을 함께 판단하는 **Dynamic Intent Orchestration / Autonomous Task Planning** 계층이다. 즉, "착신전환이 어디에 있나요?"와 "착신전환을 010-1234-5678로 바꿔줘"는 같은 주제를 말하지만, 전자는 안내가 목적이고 후자는 변경 실행이 목적이므로 전혀 다른 처리 경로를 선택한다.

#### 유형 분류 뒤에 이루어지는 다섯 가지 판단

유형은 정답이 아니라 다음 행동을 정하기 위한 출발점이다. IntelliDecision은 매 발화마다 아래 다섯 질문에 답한 뒤 응답을 구성한다.

| 판단 축        | AI가 확인하는 내용                                                             | 착신전환 예시                                      | 판단 결과                                    |
| -------------- | ------------------------------------------------------------------------------ | -------------------------------------------------- | -------------------------------------------- |
| 1. 사용자 목적 | 사용법 안내, 현황 조회, 실제 변경, 취소 중 무엇을 원하는가                     | "어디서 설정해?"는 탐색, "새 번호로 바꿔줘"는 실행 | 안내 경로 또는 실행 경로 선택                |
| 2. 이전 대화   | 이미 안내했는지, 어떤 값과 조건을 수집했는지, 직전 실행은 무엇이었는가         | 화면 안내 뒤 "그럼 설정해줘"라고 말함              | 새 요청으로 끊지 않고 탐색에서 실행으로 전환 |
| 3. 필요한 정보 | 처리를 위해 대상, 값, 기간, 본인 확인, 영향 범위 중 무엇이 빠졌는가            | 전환 대상 회선은 알지만 새 번호가 없음             | 필요한 값만 한 가지씩 질문                   |
| 4. 실행 위험도 | 읽기인지 쓰기인지, 다수 대상인지, 되돌릴 수 있는지, 정책상 허용되는지          | 여러 회선에 영향을 주는 번호 변경                  | 즉시 실행 대신 영향 범위와 승인 요구         |
| 5. 다음 행동   | 안내, 근거 검색, 조회, 확인 질문, 승인 요청, 실행, Undo, 이관 중 무엇이 맞는가 | 번호와 권한이 확인되고 최종 승인까지 받음          | 승인된 변경만 실행하고 결과를 검증           |

이 판단은 한 번 하고 끝나지 않는다. 사용자가 설명을 듣다가 실행을 요청하거나, "아니요, 다른 번호로요"라고 정정하거나, 실행 후 "방금 변경을 취소해줘"라고 하면 매 턴마다 목적과 위험도를 다시 평가한다. 따라서 정해진 스크립트를 끝까지 강제하는 대신, 필요한 정보만 수집하고 대화의 변화에 맞춰 안전하게 경로를 바꾼다.
...
flowchart TD
    ...
    T --> P{무엇이 목적인가?}
    P -->|안내| K[근거·화면 경로 탐색]
    P -->|조회·변경| H{정보·권한이 충분한가?}
    H -->|아니오| Q[부족한 정보만 질문]
    Q --> U
    H -->|예| R{실행 위험도와 정책은 허용되는가?}
    R -->|낮음·조회| S[조회 후 결과 설명]
    R -->|변경 가능| C[영향 범위 설명·최종 승인]
    C --> E[승인된 실행·결과 검증]
    R -->|불가·고위험| X[안내 또는 담당자 이관]
    K --> A[근거 있는 안내·다음 선택 제안]
    S --> A
    E --> A
```
...
#### 유형별 Statement - 유형을 실제 대화 행동으로 바꾸는 기준

각 유형에는 단순 분류명이 아니라 AI가 따라야 할 행동 원칙이 있다. 아래 Statement는 응답 문구를 외우는 규칙이 아니라, 어떤 정보를 찾고 어떤 질문을 하며 언제 실행을 멈춰야 하는지를 정하는 운영 기준이다.

| 유형           | 판단 Statement                                            | AI가 따르는 행동                                                            | 응답 예시                                                                                     |
| -------------- | --------------------------------------------------------- | --------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| A. 탐색        | 사용자는 실행보다 이해를 원한다.                          | 매뉴얼·화면·사전 조건을 근거로 설명하고 실행하지 않는다.                    | "설정 메뉴 → 통화 제어에서 확인할 수 있습니다. 현재 설정도 확인해 드릴까요?"                  |
| B. 실행        | 사용자는 실제 상태 확인 또는 변경을 원한다.               | 대상·필수 값·권한을 확인하고, 변경이면 영향 범위와 최종 승인을 받는다.      | "A회선의 착신 번호를 010-1234-5678로 바꾸는 것이 맞을까요?"                                   |
| C. 포괄 도움   | 사용자는 기능 하나가 아니라 가능한 선택지를 알고 싶다.    | 여러 업무 영역을 찾아 요약하고, 가장 관련 높은 다음 질문을 제안한다.        | "설정 안내, 통화 이력 조회, 착신 규칙 변경을 도와드릴 수 있습니다. 어느 업무가 필요하신가요?" |
| D. 정정        | 이전에 수집한 값 또는 목표가 바뀌었다.                    | 이미 맞는 정보는 유지하고 바뀐 항목만 교체해 재확인한다.                    | "알겠습니다. 기존 번호 대신 010-9876-5432로 변경하겠습니다. 이 번호가 맞나요?"                |
| E. 실행 취소   | 사용자는 완료된 변경을 되돌리고 싶다.                     | 직전 실행과 복원 가능 상태를 확인한 뒤, 가능한 경우에만 복원 승인을 받는다. | "방금 변경한 착신 번호를 이전 값으로 되돌릴 수 있습니다. 진행할까요?"                         |
| F. 모호성 해소 | 요청만으로 대상 또는 행동을 하나로 정할 수 없다.          | 추측해 실행하지 않고 가장 적은 수의 선택 질문으로 범위를 좁힌다.            | "어느 회선의 착신전환을 말씀하시나요? 번호 또는 회선명을 알려주세요."                         |
| G. 일괄 처리   | 하나의 요청이 여러 대상에 영향을 준다.                    | 대상 목록·건수·예상 결과를 먼저 제시하고 전체 승인을 받는다.                | "3개 회선의 번호가 변경됩니다. 대상 목록을 확인한 뒤 진행할까요?"                             |
| H. 범위 외     | 서비스가 처리할 수 없거나 정책상 실행할 수 없는 요청이다. | 불가능한 이유와 가능한 대안을 설명하고 적절한 담당 채널로 이관한다.         | "이 요청은 현재 자동 처리 범위 밖입니다. 담당 지원 채널로 연결해 드리겠습니다."               |
| I. 반복        | 사용자는 방금 들은 내용을 다시 확인하려 한다.             | 기존 답변의 맥락과 현재 단계는 유지한 채 더 짧고 명확하게 재표현한다.       | "다시 안내드리면, 설정 메뉴 → 통화 제어에서 착신 규칙을 선택하시면 됩니다."                   |
...
sequenceDiagram
    ...
    participant T as Dynamic Tool Wrapper

    U->>I: 착신전환은 어디서 설정해?
    I->>I: A 탐색 판단 - 실행 요청 아님
    I->>R: 화면 경로·사전 조건 탐색
    R-->>I: 통화 제어 메뉴, 권한·변경 조건
    I-->>U: 설정 메뉴 -> 통화 제어입니다. 설정도 도와드릴까요?
    U->>I: 네, A회선을 010-1234-5678로 바꿔줘
    I->>I: B 실행 판단 - 대상·값·영향 범위 확인
    I->>T: 허용된 변경 수단과 현재 상태 확인
    T-->>I: 변경 가능, 최종 승인 필요
    I-->>U: A회선의 착신 번호를 변경합니다. 진행할까요?
    U->>I: 네
    I->>I: 최종 승인 확인 - 실행 조건 충족
    I->>T: 승인된 변경 실행
    T-->>I: 원격 상태 검증 완료
    I-->>U: 변경을 완료했습니다. 필요하면 이전 설정으로 되돌릴 수 있습니다.
```
...
**시장 사례 1 - Anthropic Routing: Claude를 만드는 회사가 제시한 에이전트 설계 방식.** Anthropic은 Claude 모델과 Claude API를 제공하는 AI 기업이다. Routing은 별도 소비자 서비스의 이름이 아니라, Anthropic이 Claude API로 고객 지원·콘텐츠 검토·데이터 처리 에이전트를 설계할 때 권장하는 대표 아키텍처 패턴이다. Claude를 활용하는 기업은 들어온 요청을 일반 질문, 환불 요청, 기술 지원처럼 먼저 구분한 뒤, 각각에 맞는 전문 프롬프트·후속 프로세스·도구로 보낸다. 예를 들어 일반 질문은 빠른 지식 답변으로, 환불 요청은 주문 정보 조회와 정책 확인으로, 기술 지원은 진단 절차 또는 담당자 이관으로 연결한다.

이 방식의 핵심은 모든 요청을 하나의 거대한 프롬프트나 단일 시나리오로 처리하지 않는 것이다. 한 유형에 최적화한 응답 규칙이 다른 유형의 정확도를 떨어뜨리는 문제를 줄이고, 필요한 경우에만 적합한 Tool을 연결한다. IntelliDecision은 이 Routing 원칙을 통화매니저 업무에 맞게 A-I 유형과 다섯 판단 축으로 구체화한다. 사용법 질문은 지식 탐색으로, 설정 변경 요청은 승인된 실행 준비로, 불명확한 요청은 보완 질문으로 보내며, 사용자가 대화 중 목적을 바꾸면 경로도 다시 선택한다.
...
**시장 사례 2 - Google Dialogflow CX: Google Cloud의 기업용 대화형 AI 구축 서비스.** Dialogflow CX는 Google Cloud가 제공하는 엔터프라이즈급 음성봇·챗봇 구축 서비스다. 기업은 이 서비스를 이용해 콜센터 IVR, 웹 채팅, 모바일 앱 상담봇을 설계하고 운영한다. 대화 주제를 Flow로, 현재 단계와 수집해야 할 정보를 Page로, 사용자 발화 또는 세션 조건에 따른 이동을 Route로 관리한다. 예를 들어 항공사·통신사·금융사의 고객 지원 흐름에서 주문·고객 정보·최종 확인을 별도 Page로 두고, 사용자가 필요한 정보를 주면 다음 Page로 이동시키며, 조건이 바뀌면 다른 Route로 전환한다.

이 서비스가 보여 주는 핵심은 실제 기업 대화 시스템이 "의도 하나 → 답변 하나"가 아니라, 의도와 현재 세션 상태를 함께 보며 다음 단계를 결정한다는 점이다. Dialogflow CX는 이를 운영자가 그래픽 콘솔에서 사전에 설계한 상태 머신으로 구현한다. AI Service Agent는 고정된 화면 설계 도구 대신 IntelliDecision이 실행 시점에 이 역할을 수행한다. 따라서 "어디서 설정해?"에서 "설정해줘"로 바뀌는 요청을 새 대화로 끊지 않고, 탐색에서 실행으로 자연스럽게 전환한다.
...
현재 시스템은 A-I 유형, 대표 발화, 지식 탐색 필요 여부, Tool 필요 여부를 정책으로 관리한다. 이 정책은 단순 분류 결과가 아니라 다음 단계의 검색 범위, 보완 질문, 실행 가능 경로를 결정하는 기준이다.
...
| 업무·설정 도메인       | 통화 제어, 채팅 자동응답                    | 문서와 화면·정책을 연결하는 중심 노드 | 실제 사용 |
...
현재 시스템은 문서·도메인·화면·IntelliDecision 유형 간 관계를 순회하고, 포괄 도움 요청에는 여러 도메인을 병렬 검색한다. Full GraphRAG의 자동 엔터티 추출은 현재 운영 규모에 비해 과도해 채택하지 않았으며, API endpoint·절차 단계 관계는 후속 확장 대상으로 남겨 두었다.
...
현재 시스템은 테넌트별 OpenAPI 문서에서 동적 Tool을 만들고, GET은 조회용으로 노출하며 쓰기 method는 서비스 오너가 승인한 경우에만 노출한다. 실행 단계에서는 승인 검사와 가능한 사전 상태 저장, HTTP 실행, 실행 로그 기록, 원격 상태 대조 및 Undo 경로를 함께 처리한다.
