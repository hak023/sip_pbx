...
| 실행 정책    | 로그인한 고객의 본인 회선 확인, 새 번호 확인, 최종 승인, Undo 가능 여부 | 변경 요청을 안전하게 통제할 때                 |
...
IntelliDecision은 단순 분류기가 아니다. 매 발화마다 고객의 목적, 앞선 대화, 누락된 정보, 실행 위험, 다음 행동을 함께 판단한다. 이 체계 아래에서 LLM은 정해진 대본을 반복하는 대신 현재 대화에 맞는 지식과 행동을 선택한다. 앞 장의 UI 입력·LLM 처리·UI 응답 그림에 IntelliDecision을 더하면 다음과 같다.
...
flowchart LR
    ...
    I[IntelliDecision\n목적·맥락·다음 행동 판단] --> L
    L --> D{이번 질문은?}
    D -->|가능 여부·사용법| R[UI에 화면 안내 표시]
    D -->|설정 요청·정보 부족| F[UI에 필요한 정보·확인 요청]
    D -->|정정·취소·범위 밖| H[UI에 수정·Undo·이관 안내]
    F --> U
    R --> U
    H --> U
```
...
N-hop RAG는 첫 문서 검색 결과에서 멈추지 않는다. 관련 문서에서 업무 영역으로, 업무 영역에서 화면 안내와 정책으로 한 단계씩 연결해 답변 근거를 넓힌다. 고객은 “매뉴얼에 적힌 문장”이 아니라 지금 해야 할 행동을 안내받는다. 앞 그림의 IntelliDecision이 안내 요청으로 판단한 경우, N-hop RAG가 근거를 더해 다음 그림처럼 응답을 완성한다.
...
착신전환 예시에서 AI는 먼저 현재 규칙을 조회하고, 로그인한 고객 본인에게 연결된 회선인지 확인한다. 다른 사람의 회선은 조회·변경 후보에 포함하지 않는다. 이어 고객이 확인한 전화번호와 시간 조건을 정리하고, 사전에 승인된 변경 API만 후보로 삼아 최종 승인을 요청한다. 승인 뒤에는 API를 실행하고, 응답만 믿지 않고 원격 시스템의 실제 상태를 다시 확인한다. 앞 그림에서 고객이 “설정해 주세요”라고 답한 경우, Dynamic Tool Wrapper가 마지막 실행 단계를 더한다.
...
flowchart LR
    ...
    I[IntelliDecision\n실행 요청·필수 정보 판단] --> L
    L --> N[N-hop RAG\n규칙·화면·실행 정책 확인]
    N --> T[Dynamic Tool Wrapper\n본인 회선·권한·승인 API 확인]
    O[OpenAPI 명세·인증·허용 메서드] --> T
    T --> C[UI에 변경 내용·최종 승인 요청]
    C -->|승인| E[본인 회선 설정 API 실행]
    E --> V[원격 상태 재확인]
...
| 권한과 정책  | 로그인한 고객 본인 회선 범위, 인증, 승인된 메서드인지 확인                        |
