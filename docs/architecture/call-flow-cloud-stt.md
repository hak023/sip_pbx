...
1. **AI 봇 인사말 응대**
   - 발신 유저의 호가 인입되면 유저에게 연결하기 전 AI 통화비서가 미디어 TGW를 통해 발신 유저에게 인사말을 전송합니다. ("어떤 용무로 전화했는지 말씀해주세요.")
   - 발신 유저가 인사말에 응답하여 용무를 말하면("시세보다 저렴한 상가가 있어 연락드렸습니다."), 해당 음성은 미디어 TGW를 거쳐 AI 통화비서로 전달됩니다.
   - AI 통화비서는 인입되는 음성을 STT 서버에 전달하여 텍스트로 전사하고, 전사된 텍스트를 AI Agent가 넘겨받아 스팸성 내용인지 판단합니다.

2. **스팸 여부 판단 및 호 연결 (분기)**
   - **정상 통화일 경우 (스팸 아님):** AI Agent는 AI 통화비서에 '일반 통화 이벤트'를 전달합니다. AI 통화비서는 통화매니저 AS에 착신연결(정상통화) 이벤트를 전달하며, 통화매니저 AS는 별도의 경고 멘트 송출 없이 교환기를 통해 즉시 착신 유저와 호를 연결(Establish)합니다.
   - **스팸성 멘트를 확인할 경우:** AI Agent는 AI 통화비서에 '스팸 의심 이벤트'를 전달합니다. AI 통화비서는 통화매니저 AS에 착신연결(스팸의심)을 알립니다. 통화매니저 AS는 교환기를 통해 착신 유저와 연결을 시도하고, 미디어 서버(MS)와 TGW를 통해 착신 유저에게 **"스팸의심 전화입니다."** 라는 안내 멘트를 송출합니다.
