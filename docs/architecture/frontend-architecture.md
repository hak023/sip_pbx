...
프론트엔드는 **내선번호(Extension) 기반 로그인** 방식으로 멀티테넌트를 지원합니다. 로그인 시 내선번호를 입력하면 해당 테넌트의 설정이 자동으로 로드되며, 모든 페이지에서 테넌트별 데이터만 표시됩니다.

**테넌트 컨텍스트 관리**:
- 로그인 시 `localStorage`에 tenant 정보 저장: `{ owner: "1003", name: "이탈리안 비스트로" }`
- 모든 API 호출에 `owner` 또는 `callee` 파라미터 자동 포함
- 페이지 로드 시 `localStorage`에서 tenant 정보 확인, 없으면 로그인 페이지로 리다이렉트

### Application Types (구현 완료)

**1. 로그인** (`/login`)
- 내선번호(Extension) 기반 로그인
- 테넌트 설정 자동 로드 (VectorDB에서 조회)
...
**3. 지식 관리** (`/knowledge`)
- 테넌트별 Knowledge Base CRUD
- 지식 검색, 추가, 수정, 삭제

**4. AI 서비스** (`/ai-services`)
- 테넌트별 AI 기능(Capability) 관리
- 테넌트 설정 (인사말, 시스템 프롬프트) 편집

**5. 통화 이력** (`/call-history`)
- 테넌트별 CDR 조회 (callee 기준 필터)
- 녹음 재생, Transcript 확인
...
## Project Structure (구현 완료 - Next.js App Router)
...
✅ **멀티테넌트 지원**: 내선번호 기반 로그인, localStorage 테넌트 컨텍스트, API owner 필터  
✅ **Key Features** (구현 완료):
- 내선번호 기반 로그인 (테넌트 자동 식별)
- 테넌트별 대시보드 (통화 통계, Knowledge 통계)
- 테넌트별 지식 관리 (Knowledge CRUD)
- 테넌트별 AI 서비스/기능 관리
- 테넌트별 통화 이력 (CDR 조회)
- 테넌트별 지식 추출 이력
...
| 문서 | 설명 |
|------|------|
| [technical-architecture.md](./technical-architecture.md) | 전체 시스템 기술 아키텍처 |
