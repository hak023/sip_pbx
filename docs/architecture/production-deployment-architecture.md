...
* **기준 시나리오 (Busy Hour, §6.4)**
  ...
  * **STT 호출:** 모든 통화에서 실시간 화면 표시(CID) 및 AI 응대용으로 상시 구동
...
* **API/Realtime 단독 한계(보충):** 본 표에서는 **통화 1건당 장기 WSS 구독 1건(1:1)** 을 가정합니다. 이 경우 동시 통화 상한은 **벤치 동시 WSS(예: Active 1대 기준 5,000)** 와 **동일**이며, `25,000`처럼 WSS를 임의로 곱해 올리지 않습니다.
...
| **STT** | 동시 스트림 | 1,250 | 약 770 (전체 호) | 694호 (호당 1.8스트림) | 20,820명 | 2 |
...
- 시나리오 가정:
  ...
  - **STT 사용:** AI 호 및 유저간 통화 모두 실시간 CID용으로 상시 사용 (호당 평균 1.8 스트림 가정; 유저간 2스트림, AI 1스트림)
  - **TTS 사용:** AI 호에서만 사용
  - 유저는 `1시간에 1회 통화`
