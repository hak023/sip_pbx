...
|------|-----------------------------------|----------------------------------|------|
| LLM | **25** inf QPS (120 tok/req, 2노드 합); 운영 **80% → 20** QPS | **20** QPS (평균 0.0467 QPS/호) | AI 구역 병목; **전체 428 동시호(AI 85호)** 수용 가능 |
| AI Runtime | **500** 동시 세션(250/노드×2, 벤치) | **85** 동시 세션(활성 AI 호 1:1) | 오케스트레이션 단위 (여유) |
| STT | **1,250** 동시 스트림(625/노드×2) | **~770** 동시 스트림(유저간 2, AI 1) | 미디어 평면 (여유) |
| TTS | **2,000** 동시 합성(1,000/노드×2) | **≤85** 동시(턴·합성 겹침 피크, 보수) | §5.2; (큰 여유) |
...
**최대 수용 AI 동시호**와 **최대 수용 가입자**는 **해당 노드의 벤치 상한만으로 역산했을 때** 수용 가능한 이론적 한계치다. 이를 통해 **어느 노드가 전체 시스템의 병목인지(LLM)** 파악한다.

| 노드 | 단위 | 벤치 상한 (클러스터·요약) | §6.4 BH 시나리오 기대 부하 | 최대 수용 AI 동시호 (단독 한계) | 최대 수용 전체 동시호 (단독 한계) | 최대 수용 가입자 (명, 단독 한계) | 대수 |
|------|------|---------------------------|---------------------------|---------------------------------|-----------------------------------|----------------------------------|------|
| LLM | inf QPS | **25** (운영 **20**) | **20** | **85** | **428** | **12,840** | 2 |
| AI Runtime | 동시 세션 | **500** | **85** | **500** | **2,500** | **75,000** | 2 |
| STT | 동시 스트림 | **1,250** | **770** (전체 호) | **1,250** | **694** (1.8스트림/호) | **20,820** | 2 |
| TTS | 동시 합성 | **2,000** | **≤85** (피크) | **2,000** | **10,000** | **300,000** | 2 |
| API/Realtime | 동시 WSS | **5,000** (가정, §5.5) | 연동 모델 의존 | **5,000** (1:1 가정 시) | **25,000** | **750,000** | 1+1 |
| AIR GW | 시그널 TPS | 제품·프로파일 벤치 | **~3.56** CPS권 + 세션 이벤트율 | — | — | — | 1+1 |
