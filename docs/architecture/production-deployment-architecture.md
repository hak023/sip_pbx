...
| 동시세션 산정(가상 시나리오) | `AI Runtime 2노드(동시 세션 500 벤치)·통화 1:1 세션 가정 시 전체 ≈500 동시호 (AI ≈100호). STT·8B급 LLM 단독 한도는 그보다 큼` |
...
본 문서는 기존 통신 인프라(교환기, 통화매니저AS, WTIMS)에 AI Call Agent 시스템을 연동하기 위한 상용 아키텍처 설계서입니다.
이 장에서는 **기준 시나리오(유저당 1시간 1통화, 평균 120초, AI 응대율 20%)**를 바탕으로 산출한 핵심 아키텍처, 서버 구성, 용량 지표 및 예상 비용을 요약하여 제공합니다. 상세한 연동 규격과 비용 산출 근거는 본문에서 확인할 수 있습니다.

**읽는 순서(권장):** (1) 본 빠른 참조 → (2) 설계 전제(1절)·전체 배포도(2절) → (3) 프로토콜 요약(3절) → (4) 시퀀스·역할 스펙(4~6절) → (5) DB·EMS·체크리스트·결론(7~10절) → (6) 비용 상세(11절: 하드웨어 CAPEX·SW 개발비·기존 연동비) → (7) 부록 A 연동 예제

### 1. 아키텍처 요약

**핵심 요약:** 기존 통신 코어(교환기 → 통화매니저AS → WTIMS)를 그대로 유지합니다. WTIMS는 RTP 미러링을 통해 STT 풀(All-Active)로 음성을 직접 전달합니다. 세션 제어를 위한 통합 시그널은 단일 진입점인 **AIR GW(Active/Standby)**를 거쳐 일관 해시(Consistent Hash) 방식으로 **AI Runtime 풀(All-Active)**에 분배됩니다. 내부의 STT, LLM, TTS 풀 역시 모두 All-Active로 구성되어 확장성을 확보합니다. 외부 연동(유엔젤, 바이토)은 API/Realtime 단일 VIP를 통해서만 이루어집니다.
...
*(전체 배포도 및 관제(EMS) 포함 상세 다이어그램은 2절 참조)*

### 2. 통화 및 데이터 플로우 요약

1. SIP `INVITE`가 교환기를 거쳐 통화매니저AS와 WTIMS(RTP 앵커)로 인입됩니다.
2. 사용자 음성(RTP)은 WTIMS에서 복제되어 **GW를 거치지 않고 직접 STT로 전달**됩니다.
3. 통화매니저AS는 호 세션 정보를 WTIMS로 릴레이하고, WTIMS는 이를 통합 시그널로 묶어 **AIR GW(단일 VIP)**로 전송합니다. GW는 전용 L4 스위치 없이 일관 해시를 통해 **AI Runtime**으로 부하를 분산합니다.
4. AI Runtime은 STT 스트림을 읽고 LLM을 호출한 뒤, 생성된 텍스트를 TTS를 통해 음성(RTP)으로 변환하여 WTIMS로 재생합니다.
5. 외부 API(유엔젤/바이토)는 API/Realtime 단일 VIP를 통해 인바운드 요청을 처리하며, 아웃바운드 요청은 AI 시스템에서 코어망으로 전송합니다.

### 3. 서버 구성 요약 (EMS 제외, 용량 상한 기준)

**신규 AI Call Agent 계층 합계: 16노드** (가상 시나리오 기준. EMS, 교환기, 통화매니저AS 제외)

| 구분 | 핵심 내용 |
|------|------|
| **처리 용량 목표** | 시간당 15,000명 수용 (약 4.17 CPS) / 평균 통화 시간 120초 |
| **최대 동시 세션** | 전체 **500호** (이 중 AI 응대 **약 100호**) ※ **AI Runtime 동시 세션(500, 통화 1:1)** 병목 기준(§6.1). STT·8B급 LLM은 동일 부하에서 여유 |
| **기존 활용 자산** | 교환기, 통화매니저AS(A/S), WTIMS RTP 서버, 통화매니저 API(유엔젤/바이토) |
| **신규 구축 시스템** | **AI Call Agent (STT, TTS, LLM, AI Runtime, API, DB)** 및 **통합 관제(EMS)** |
| **고가용성(HA) 구조** | 진입점(AIR GW, API VIP) 및 DB는 **Active/Standby**, 그 외 AI 코어 모듈은 모두 **All-Active**로 구성 |

*(서버별 상세 권장 사양(CPU/RAM/GPU 등)은 **6.3절** 참조)*

### 4. 성능 및 용량 산정 기준

가정·산식·500/100호·CPS 등 **가상 시나리오의 정본**은 본문 **`## 6) 용량·서버 산정`** 안의 **`### 6.1 가상 시나리오 처리량 환산 (유저 수)`** 절에 둔다(표·빠른 참조는 그 결과를 요약).

시스템의 **실효 동시 통화 상한**은 **STT·LLM·Runtime 등 구성 요소 중 가장 먼저 포화되는 병목**으로 결정된다. 본 구성(§6.1)에서는 **AI Runtime(500호)** 이 실효 상한이며, **8B급 LLM(L40S×1/노드)** 은 **500호 시 운영 QPS 약 76%** 로 **STT(900/1,250 스트림)보다 여유가 좁다**.

* **기준 시나리오 (Busy Hour, §6.1)**
  * **사용자 패턴:** 1시간당 1회 통화, 평균 120초 유지, **10초당 1회 발화**
  * **통화 비율:** 전체 통화 중 AI 응대 20%, 일반 유저간 통화 80%
  * **LLM 호출:** AI 통화는 발화당 **1.2회** 질의(호당 **14.4회**), 유저간 통화는 종료 시 요약용(호당 1회) 호출
  * **STT 호출:** 모든 통화에서 실시간 화면 표시(CID) 등으로 **RTP 미러 기반 연속 인식**이 필요하다고 본다.
  * **STT 벤치 단위(중요):** 문서의 **625/노드·클러스터 1,250** 은 **「통화 1건」이 아니라 「동시 ASR 스트림 1개」** 를 1단위로 부하 테스트·벤치에 적재했을 때의 상한이다. 즉 **RTP 미러(또는 동등)로 STT에 붙는 디코딩 채널 1개 = 스트림 1개**이다.
  * **호당 스트림 수(가정):** 유저간 통화(80%)에서는 발·착 **레그별 CID**를 위해 **스트림 2개/호**, AI 응대 호(20%)에서는 **스트림 1개/호**(대표 레그 1본만 인식)로 두었다. 통화 비율로 가중하면 **0.8×2 + 0.2×1 = 1.8 스트림/호(평균)** 이다. 실제 제품이 **혼합 1채널**이거나 **한쪽만 CID**면 호당 1.0 등으로 다시 잡고, **합산 기대 스트림(동시) = 동시 통화 × 호당 스트림**으로 곱하면 된다.

* **성능 평가 방식**
  * **이론적 최대 성능 (벤치마크):** 각 노드가 개별적으로 처리할 수 있는 최대 부하량
  * **호당 부하량(매트릭스):** 위 기준 시나리오(§4·§6.1)에서 **통화 1건**이 해당 노드에 거는 부하의 **대표값**이다. **평균 통화 1호**로 쓸지 **AI 호 / 유저간 호**로 나눌지는 노드마다 다르며, **동시 호 수 × 호당 부하 ≈ §6.2·§6.4의 역할별 합산**과 맞는지로 검증한다.
  * **최대 수용 가능 전체 동시호:** 벤치 상한을 호당으로 나눈 **단독 병목 역산**이므로, 위 **호당 부하량**과 곱해 보면 시나리오 합산과 교차 검증할 수 있다.

### 5. 노드별 성능 및 수용량 매트릭스

아래 표는 각 노드의 **이론적 최대 성능**을 바탕으로, 해당 노드 단독으로 감당할 수 있는 **최대 동시 통화 수**와 가입자 수를 역산한 결과입니다. **호당 부하량** 열은 같은 가상 시나리오에서 **통화 1건**이 그 노드에 걸리는 부하만 따로 적어 두었고, 필요하면 **동시 호 × 호당**으로 **합산 부하**를 떠올리면 됩니다. **실제 상한**은 표의 **최대 수용 가능 전체 동시호** 중 **가장 작은 값(본 구성에서는 STT)** 이 된다.

**[표 필드 상세 설명]**
* **시스템 노드:** AI Call Agent 아키텍처를 구성하는 각 핵심 서버 또는 모듈입니다.
* **성능 측정 단위:** 해당 노드의 처리량을 평가하는 기준 지표입니다 (예: 초당 쿼리 수(QPS), 동시 연결 세션 수, 스트림 수 등).
* **이론적 최대 성능 (벤치마크):** 비즈니스 시나리오와 무관하게, 해당 노드(클러스터)가 하드웨어 스펙상 순수하게 감당할 수 있는 최대 부하량 또는 부하 테스트 한계값입니다.
* **호당 부하량 (가상 시나리오, §6.1과 동일 가정):** **통화 1건**이 해당 노드에 거는 부하이다. **평균 통화 1호**(AI 20%·유저간 80% 가중)로 적은 값과, **호 유형별**(AI만 해당·유저간만 해당)로 적은 값이 섞여 있을 수 있으니 각 칸의 괄호를 본다. 단위는 **성능 측정 단위** 열과 같다.
* **최대 수용 가능 전체 동시호:** 다른 노드의 성능은 무한하다고 가정할 때, **오직 해당 노드의 성능 한계만으로** 감당할 수 있는 시스템 전체의 최대 동시 통화 수입니다. **행마다 다른 “단독 역산”**이며, 본 구성(§6.1·§6.2)에서는 **AI Runtime(500호)** 가 가장 작아 **실제 동시 통화 상한**을 결정한다(통화 **1건당 Runtime 세션 1개**·§5.4). **STT(약 694호)**·**8B급 LLM(약 652호)** 은 그보다 크나, **500호** 부하에서 LLM 운영 QPS 여유는 **STT·Runtime보다 좁다**(§6.4).
* **STT 단독 한계(보충):** 벤치 **1,250** 은 **동시 스트림** 상한이다. 표의 **「694호」** 는 **호당 평균 1.8스트림**일 때만 성립한다(`1,250 ÷ 1.8 ≈ 694` 동시 통화). 호당 **1스트림**이면 STT 단독 한계는 **약 1,250동시 통화**까지 올라간다(다른 병목 전제).
* **API/Realtime 단독 한계(보충):** 본 표에서는 **통화 1건당 장기 WSS 구독 1건(1:1)** 을 가정한다. 이 경우 동시 통화 상한은 **벤치 동시 WSS(예: Active 1대 기준 5,000)** 와 동일하다.
* **최대 수용 가능 가입자 (시간당):** 해당 노드가 감당할 수 있는 최대 동시호를 바탕으로, 1시간 동안 수용 가능한 총 가입자 수를 환산한 수치입니다 (가입자당 1시간에 1통화, 평균 120초 유지 기준).
* **구성 대수:** 해당 성능을 내기 위해 배치되는 서버의 수와 이중화 방식(All-Active 또는 Active/Standby)입니다.

| 시스템 노드 | 성능 측정 단위 | 이론적 최대 성능 (벤치마크) | 호당 부하량 (가상 시나리오, §6.1) | 최대 수용 가능 전체 동시호 | 최대 수용 가능 가입자 (시간당) | 구성 대수 |
|-------------|----------------|-----------------------------|-----------------------------------|----------------------------|--------------------------------|-----------|
| **LLM** | 추론 QPS | **25** (운영 상한 **20**, §5.3·§6.1) | 평균 **≈0.031 QPS/호** `(0.2×14.4+0.8×1)/120` · AI **0.12** · 유저간 **≈0.008** | **652호** (AI **130호**, LLM 단독 역산) | **19,560명** | 2 |
| **AI Runtime** | 동시 세션 | 500 | **1 세션/호** (유저간·AI 공통; CID·실시간 STT 연계 오케스트레이션) | **500호** | **15,000명** | 2 |
| **STT** | 동시 스트림 | 1,250 | 평균 **1.8 스트림/호** · 유저간 **2** · AI **1** | **694호** (호당 **1.8**스트림 가정) | 20,820명 | 2 |
| **TTS** | 동시 합성 | 2,000 | AI **≤1 합성/호(피크, 보수)** · 유저간 **0** | 10,000호 | 300,000명 | 2 |
| **API/Realtime**| 동시 WSS | 5,000 (Active 1대 벤치 가정, §5.5) | **1 WSS/호** (통화·장기 구독 **1:1** 가정) | **5,000호** (통화·WSS 1:1이면 벤치와 동일) | **150,000명** (5,000×30, 1콜/h·120초) | 1+1 |
| **AIR GW** | 시그널 TPS | 제품별 벤치마크 필요 | **호당 단일 수치 비권장** — 신규 세션·이벤트율은 **CPS·프로파일(§6.1·SW)** | - | - | 1+1 |
| **Qdrant** | 검색 QPS | 약 200 | **§7.2** 경로별; 예시 **≈0.2 QPS/AI호** 가정 시 §6.2 합산과 맞춤 | 5,000호 | 150,000명 | 2 |
| **PostgreSQL** | TPS / 커넥션 | 벤치마크 필요 | **경로별 (§6.1·§7)** — 호당 단일 TPS로 고정하지 않음 | - | - | 2 |

*(검증: **실효 상한 500호**일 때 STT는 **500×1.8=900** 스트림, LLM은 **500×0.03067≈15.3 QPS**(운영 **20 QPS** 대비 **약 76%** — 폭언 2차 LLM·피크 시 벤치 재확인). **Runtime 증설(3노드 이상)** 또는 **세션 공유/경량 경로**로 통화당 세션을 줄이면 상한을 다시 올린다. 벤치는 **스트림/세션** 단위.)*

### 6. 도입 비용 요약

| 구분 | 예상 비용 (ROM) | 비고 및 상세 참조 |
|------|-----------------|-------------------|
| **하드웨어 인프라 (CAPEX)** | **약 2.4억 ~ 3.6억 원** | AI Call Agent 16노드 기준 (11.2절, LLM **L40S×1**/노드) |
| **AI Call Agent SW 개발비** | **약 9.36억 원** (72.0 MM) | 순수 소프트웨어 개발 인건비 (11.4절) |
| **기존 시스템 연동 개발비** | **약 0.7억 원** + α | WTIMS 연동 기준, 기타 연동비 별도 산출 (11.3절) |
| **(참고) 타사 솔루션 도입 시**| 약 2.9억 ~ 3.38억 원 | 자체 STT 개발(약 1.17억 원)과 비교용 (11.5절) |
...
- **동시세션(운영 기준)**: **500호** (전체 호), **약 100호** (AI 호) (Busy Hour 기준, §6.1)
- **CPS**: 약 **4.17** INVITE/s 지속 유입
- **AI 적용률 가정**: 20% (전체 호 중 20%만 AI 응대)
- **실시간 AI 활성 구간 가정**:
  - STT 활성 세션: `~900` (500호×1.8, 전체 호 CID·AI 응대, §6.1)
  - TTS 동시 합성 기준: 서버 처리량 기준으로 산정(`1,000 세션/노드`, 1 Active = 1,000)
  - LLM 동시 생성 요청: §6.1 계산식 기준(발화 빈도·질의수 및 유저간 통화 종료 요약 가정)
...
본 설계·**비용 산출(11절)** 에는 **L4/L7 로드밸런서 전용 장비(CAPEX)** 를 포함하지 않는다. **통화매니저 API 서버(유엔젤·바이토)** 가 API/Realtime을 호출할 때는 **단일 접점 VIP/FQDN 1개**로만 수렴시키되, VIP는 **정상 시 Active 노드**에 부착하고 **Standby**는 동일 스펙으로 대기한다. 장애 시 VIP 전환(예: Keepalived/VRRP)·DNS 스위치·운영 수동 전환 등으로 Standby가 인수한다. 정상 시 **동시 WSS·REST 수용량**은 **Active 1대 기준**으로 **벤치 상한**을 두고, **§6.1 시나리오에서 기대되는 rps·WSS**와 대조해 여유를 본다(§6.4).
...
- **처리량 가정(벤치)**: **625 동시 스트림/노드** — 여기서 **스트림 1개 = RTP 미러(또는 동등)로 STT에 연결된 연속 ASR 디코딩 채널 1개**이며, **통화 1건과 1:1이 아닐 수 있다**(§4·§6.1 호당 스트림 가정 참조).
- **구성**: 본 시나리오 **2노드 All-Active** → 클러스터 **1,250 동시 스트림**(벤치 합산). §6.1 기준 기대 **약 900 스트림**(500호×1.8) 대비 **여유** — 상한 확장 시에는 **AI Runtime·STT** 병목을 함께 본다.
...
- **구성**: 본 시나리오 **2노드 All-Active** → 클러스터 **2,000 동시**(벤치 합산). (§6.1 AI 호 기준 합성 부하 **≤100** 대비 여유)
- **기준 구분**: 위 처리량은 **벤치 기준치**이며, 운영 시에는 목표 상한을 **80% 이내(권장 800/노드)** 로 관리한다.

## 5.3 LLM 서버 (내부 구축)

- **권장 모델 (용량 산정 기준 = 저지연·8B급)**  
  - **기본 풀:** `Qwen2.5-7B-Instruct` 또는 `Llama-3.1-8B-Instruct` (**fp8**, vLLM/TGI) — 본 문서 **QPS·동시호 역산**은 이 구간을 전제로 한다.  
  - **고정밀 풀:** `Qwen2.5-14B` / `Qwen2.5-32B` 등은 **별도 노드군·별도 벤치**로 두고, 상용 용량표의 숫자와 **직접 섞지 않는다**(지연·비용·라우팅 정책 별도).
- **권장 형태**: OpenAI-compatible endpoint + vLLM/TGI
- **서버 스펙(노드당)**: 32 vCPU / 256 GB RAM / GPU **L40S 1장** / HDD 2 TB
...
- **발화/질의 가정(120초/호 기준, 재산정)**
  ...
  - `가정2` 유저 발화 간격 **10초/회** (120초 통화당 **12회 발화**)
  - `가정3` 평균 통화시간 **120초/호**
...
- **추론 처리량 가정 (8B급 fp8, L40S ×1/노드, 본 문서 산정 입력)**
  - **산정 기준 모델:** `Qwen2.5-7B-Instruct` 또는 동급 **8B** (fp8). (14B·32B는 **이 가정을 그대로 쓰면 안 된다** — tok/s가 내려가 동시 호가 줄어든다.)
  - **지속 출력:** 노드당 **1,500 tok/s** 를 **운전 가능한 중앙값**으로 둔다(= L40S **약 1,500 tok/s/GPU** × **1GPU/노드**). 스테이징에서 **p95 지연·TTFT**를 함께 보고 **1,250~1,750 tok/s/노드** 구간으로 벤치 후 확정한다.
  - **클러스터:** LLM 서버 **2노드(2 Active)** → 합산 지속 **3,000 tok/s**.
  - **요청당 출력:** **120 tokens/요청**(§가정4) 유지 시, 클러스터 이론 **QPS = 3,000 / 120 = 25**, 운영 **80%** 규칙 적용 시 **20 QPS**.
- **QPS 및 동시 call 산정 (교차 병목)**
  - call 1개당 평균 LLM 요청률: `(0.2 x 14.4회 + 0.8 x 1회) / 120초 ≈ 0.03067 req/s` (§6.1과 동일)
  - **LLM 단독 역산(클러스터):** `25 / 0.03067 ≈ 815` 이론 동시 전체 호 / 운영 `20 / 0.03067 ≈ 652` 호 (**AI 약 130호**)
  - **STT 단독 역산(스트림 1,250, 호당 1.8):** `1,250 / 1.8 ≈ 694` 동시 전체 호 (**AI 약 139호**)
  - **AI Runtime 단독 역산(세션 500, 통화 1:1):** **500** 동시 전체 호 (**AI 약 100호**)
  - **본 문서 기준 용량(실효):** `min(LLM, STT, Runtime, …)` 중 **Runtime이 먼저** → **운영 기준 500 동시 전체 세션(AI 약 100호)** 로 둔다.

## 5.4 AI Runtime 서버

- **역할**: 세션 오케스트레이션, 정책 판단, HITL, 도구 호출, STT/TTS/LLM 라우팅. WTIMS는 **AIR 연동 접점 GW**를 통해서만 유입(§1.4·§2.2).
- **용량 산정(§6.1)과의 정합:** 유저간 통화에도 **실시간 STT·CID** 연계가 필요하므로 **통화 1건당 오케스트레이션 세션 1개**로 착수한다(AI 응대 호와 동일). **가중 0.2** 같은 축약은 쓰지 않는다.
- **서버 스펙(노드당)**: 16 vCPU / 64 GB RAM / HDD 1 TB
- **NIC 권장**: **1 Gbps** (텍스트·제어 시그널 중심, 미디어 RTP 비처리)
- **처리량 가정**: **250 동시 세션/노드**(벤치); 본 시나리오는 **노드 2대 All-Active** → 클러스터 합산 **500 동시**(§6.4)
...
- **합산 부하 (§6.1):** 코어 **~4.17 CPS** 및 **인바운드 rps**는 트리거 빈도에 따라 환산한다. **통화 1건당 장기 WSS 1건(1:1)** 이면 동시 WSS는 **동시 통화 수와 동일**이며, 본 §6.1 기준 전체 **500**호일 때 **약 500** WSS(벤치 5,000 대비 여유). 호마다 WSS를 나누거나 집계 채널만 쓰는 모델이면 별도 환산한다. 빠른 참조 **호당 부하량 매트릭스·§6.4**와 맞춘다.
...
**제목의「250」:** 과거 문서 버전에서 LLM 병목으로 쓰이던 **동시 AI 통화 목표치** 예시이며, 현재 **§6.1 가상 시나리오**는 **약 100 AI 호·500 전체 호** 기준으로 읽는다(실효 상한은 **AI Runtime**, 통화 **1:1 세션**). **모든 노드의 벤치 단위를 250으로 맞추는 값은 아니다**(빠른 참조 **호당 부하량** 매트릭스·§6.4 참조).

**EMS 제외 구성·스펙:** 문서 앞쪽 **빠른 참조** 절. **벤치 상한·호당 부하량·최대 동시호:** 문서 앞쪽 **「노드별 가용 용량 매트릭스」**. **§6.2 서버 산정표**는 EMS 포함 전체이며, EMS를 빼는 구성은 **§6.5**를 본다.

### 6.1 가상 시나리오 처리량 환산 (유저 수)

- 시나리오 가정:
  - 유저당 통화 1회 `120초` 유지
  - 유저 발화 `10초당 1회` → 통화당 `12회`
  - 전체 통화 중 **AI 응대 비율 20% (유저간 통화 80%)**
  - **LLM 사용:** AI 호는 발화당 1.2회 질의(호당 14.4회), 유저간 통화는 종료 시 1회(호당 1회)
  - **STT 사용:** AI 호 및 유저간 통화 모두 실시간 CID용으로 상시 사용. **호당 동시 ASR 스트림 수**는 빠른 참조 **§4**와 같이 **유저간 2·AI 1**을 가정하면 평균 **0.8×2 + 0.2×1 = 1.8 스트림/호**이며, 동시 전체 호 **N**일 때 기대 스트림은 **약 1.8×N**이다(500호면 **900**).
  - **TTS 사용:** AI 호에서만 사용
  - 유저는 `1시간에 1회 통화`
- **AIR / AI Runtime (통화 1:1 세션):** 유저간 통화에서도 **CID·STT 연계**를 위해 **호당 AIR/Runtime 오케스트레이션 세션 1개**를 둔다(§5.4).
- **LLM (8B급 fp8, §5.3와 동일) + STT + Runtime 교차 병목**
  - **LLM 클러스터 지속 출력:** 노드당 **1,500 tok/s** × **2노드** = **3,000 tok/s** (요청당 120 tok 가정)
  - 이론 QPS: `3,000 / 120 = 25 req/s` · 운영 QPS(80%): **`20 req/s`**
  - 호 1개당 평균 LLM QPS: `(0.2 * 14.4회 + 0.8 * 1회) / 120초 = 3.68 / 120 ≈ 0.03067 QPS/호`
  - **LLM 단독 최대 전체 동시호:** `20 / 0.03067 ≈ 652`호 (**AI 약 130호**)
  - **STT 단독 최대 전체 동시호(스트림 1,250):** `1,250 / 1.8 ≈ 694`호 (**AI 약 139호**)
  - **AI Runtime 단독 최대 전체 동시호(세션 500, 통화 1:1):** **500**호 (**AI 약 100호**)
  - **본 문서가 쓰는 실효 상한:** `min(…)` 중 **500호** — **AI Runtime 세션 한도가 먼저** 도달한다.
- 전체 통화 및 가입자 환산 (AI 비율 20% 적용, **실효 500호** 기준):
  - **동시 AI 호:** `500 * 0.2 = 100`호
  - 시간당 처리 가능한 전체 통화 수 (CPS 환산): `500 동시호 x (3600 / 120) = 15,000 콜/시간` (약 **4.17 CPS**)
  - **시간당 수용 가능한 가입자 수(유저당 1콜/시간 가정): `15,000명/시간`**

*(참고: 위 **500호** 는 **AI Runtime 동시 세션(500)·통화 1:1** 로 맞춘 **실효 상한**이다. **STT만** 놓고 보면 **약 694호**, **LLM(8B)** 만 놓고 보면 **약 652호**까지 단독 역산이 가능하나 **500호** 부하에서 LLM 운영 QPS는 **약 76%** 수준이다. 각 노드별 단독 한계는 앞의 매트릭스 표를 참조한다.)*

### 6.2 서버 산정표 (§6.1 역할별 합산 vs 벤치 상한)

| 구분 | 계층 | §6.1 역할별 합산 부하 | 벤치·노드당 상한 (측정·§5) | 필요 Active 노드 | 권장 구성(여유 포함) |
|------|------|----------------------------------|---------------------------|------------------|----------------------|
| 기존 코어 | 통화매니저AS | 기존 코어 사용 | 기존 Pair 용량 기준 | 기존 2 Pair 활용 | 추가 구축 없음 |
| AI Call Agent 시스템 | AIR 연동 접점 GW | **~4.17** CPS권 시그널 + **500** 동시 호 컨텍스트 이벤트 | 제품·프로파일 **TPS 벤치** | 1 | 1 Active + 1 Standby |
| AI Call Agent 시스템 | STT | **~900** 동시 스트림(유저간 2, AI 1) | **625**/노드(벤치) | **2** | **2 Active** (All-Active; 클러스터 **1,250** 동시) |
| AI Call Agent 시스템 | TTS | **≤100** 동시 합성(턴 피크, 보수) | **1,000**/노드(벤치) | **2** | **2 Active** (All-Active; 클러스터 **2,000**) |
| AI Call Agent 시스템 | LLM | **약 15.3** QPS (500호×0.03067, 운영 **20 QPS** 대비 **약 76%**) | `1,500 tok/s×2노드`, 120 tok/req → **25** QPS 벤치 | 2 | **2 Active (fp8, 8B·L40S×1/노드)** |
| AI Call Agent 시스템 | AI Runtime | **500** 동시 세션(통화 1:1·CID·STT 연계) | **250**/노드(벤치) | **2** | **2 Active** (All-Active; GW 해시; 클러스터 **500** 동시 벤치) |
| AI Call Agent 시스템 | API/Realtime | **약 500** 동시 WSS(통화·WSS **1:1**, §6.1 전체 동시호) + 코어 **~4.17 CPS** 기반 **rps** | **5,000 WS** (가정, §5.5 참조) | 1 | **1 Active + 1 Standby** · 단일 VIP §1.6 |
| AI Call Agent 시스템 | PostgreSQL HA | 세션·이력·RAG 경로별 **TPS** (§6.1·§7) | 쓰기 Primary·풀 **벤치** | 2 | **최소** Primary + Standby(HA) |
...
| AI Call Agent 시스템 | LLM Server | 2 + 0 | 32 vCPU | 256 GB | L40S x1 | HDD 2 TB | 1 Gbps | vLLM/TGI 추론 풀 · **8B fp8** 노드당 **1,500 tok/s** 가정(§5.3) |
| AI Call Agent 시스템 | AI Runtime | **2 Active** | 16 vCPU | 64 GB | - | HDD 1 TB | 1 Gbps | 세션 오케스트레이션 · **All-Active** §2.2 |
| AI Call Agent 시스템 | API/Realtime (단일 VIP) | 1 + 1 | 16 vCPU | 32 GB | - | HDD 1 TB | 1 Gbps | REST/WSS · 벤치 상한은 §5.5; 시나리오 rps·WSS는 §6.1·연동 모델 |
...
### 6.4 목표 용량 대비 계산 체크 (벤치 vs §6.1 시나리오)

- **시나리오 축:** §6.1에서 **동시 전체 통화 500호 (AI 통화 약 100호)** 는 **AI Runtime 동시 세션(500)·통화 1:1** 에 맞춘 **실효 상한**이다. **STT** 단독 역산(**약 694호**)은 더 크고, **8B급 LLM** 단독 역산(**약 652호**)은 **500호**에 근접한다.
- **LLM:** 실효 **500호** 기준 합산 **약 15.3 QPS** / 운영 상한 **20 QPS** → **약 76%** (폭언 2차 LLM·피크·14B 이상 모델 시 **GPU 증설 또는 L40S×2/노드** 검토).
- **STT:** 시나리오 **~900** 동시 스트림 / 벤치 클러스터 **1,250** → **여유**. (상한을 **694호**까지 올리려면 **Runtime 증설**이 선행되어야 한다.)
- **TTS:** 시나리오 **≤100** 동시 합성(보수) / 벤치 **2,000** → 큰 여유(피크는 부하로 재확인).
- **AI Runtime:** 시나리오 **500** 세션 / 벤치 클러스터 **500** → **정상 부하에서 포화 인접**. 한 대 장애·피크 시 **3노드 All-Active** 또는 **세션당 부하 경량화** 검토.
- **API/Realtime:** **통화·WSS 1:1** 이면 §6.1 기준 전체 **500**호에 **동시 WSS 약 500**; 단독 병목이 될 때의 상한은 **벤치 5,000 WSS = 동시 통화 5,000호**(Active 1대 기준). 벤치는 **§5.5**에서 확정.
- **PostgreSQL·Qdrant:** 시나리오 **TPS·QPS**는 **§7·§7.2** 환산; 벤치 상한과 비교해 풀·샤딩을 맞춘다.

### 6.5 EMS 제외 시 서버 산정(옵션)
...
| 운영/부트용 HDD | **약 100,000**(1TB급) | 서버 OS/로그 기본 볼륨 기준 |
...
| LLM Server (L40S x1, fp8 가정) | 2 | 약 38,040,000 | 약 76,080,000 |
...
| **총계(EMS 제외, AI Call Agent만)** | **16노드** | - | **약 302,430,000** |

> **LLM 노드당 ROM:** L40S×2 구성(약 5,378만/노드) 대비 **GPU 1장·1슬롯 플랫폼**으로 **약 3,804만/노드** (11.1절 단가: GPU·플랫폼 차감).  
> 범위 권고: 부품·플랫폼 ROM·리셀러 할인·번들 조건에 따라 실제 발주가는 보통 **±20%** 편차가 발생한다.  
> 따라서 본 총액의 현실적 구매 범위는 **약 2.4억 ~ 3.6억원**으로 본다.
