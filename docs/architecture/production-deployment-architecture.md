...
| 항목 | 내용 |
|------|------|
| 용량 목표(가상 시나리오) | 가입자 12,840명/시간, **약 3.56 CPS(운영 기준)**, 평균 통화 유지 120초 |
| 동시세션 산정(가상 시나리오) | `LLM 2노드(fp8 1,500 tok/s/노드) 병목 기준 ≈ 85 동시 AI 호 (전체 428 동시호) 수용 가능` |
...
### 3. 서버 구성 요약 (EMS 제외, LLM 병목 기준)

**신규 AI Call Agent 계층 합계: 16노드** (가상 시나리오 기준. EMS, 교환기, 통화매니저AS 제외)

| 구분 | 핵심 내용 |
|------|------|
| **처리 용량 목표** | 시간당 12,840명 수용 (약 3.56 CPS) / 평균 통화 시간 120초 |
| **최대 동시 세션** | 전체 428호 (이 중 AI 응대 85호) ※ LLM 처리 성능 병목 기준 |
...
*(서버별 상세 권장 사양(CPU/RAM/GPU 등)은 6.1절 참조)*

### 4. 성능 및 용량 산정 기준

시스템의 최대 수용량은 가장 성능이 낮은 구간인 **LLM의 처리 한계**를 기준으로 산출되었습니다.

* **기준 시나리오 (Busy Hour, §6.4)**
  * **사용자 패턴:** 1시간당 1회 통화, 평균 120초 유지, 5초당 1회 발화
  * **통화 비율:** 전체 통화 중 AI 응대 20%, 일반 유저간 통화 80%
  * **LLM 호출:** AI 통화는 매 발화 시(호당 24회), 유저간 통화는 종료 시 요약용(호당 1회) 호출
  ...
  * **호당 스트림 수(가정):** 유저간 통화(80%)에서는 발·착 **레그별 CID**를 위해 **스트림 2개/호**, AI 응대 호(20%)에서는 **스트림 1개/호**(대표 레그 1본만 인식)로 두었다. 통화 비율로 가중하면 **0.8×2 + 0.2×1 = 1.8 스트림/호(평균)** 이다. 실제 제품이 **혼합 1채널**이거나 **한쪽만 CID**면 호당 1.0 등으로 다시 잡고, **시나리오 기대 스트림 = 동시 통화 × 호당 스트림**으로 곱하면 된다.

* **성능 평가 방식**
  * **이론적 최대 성능 (벤치마크):** 각 노드가 개별적으로 처리할 수 있는 최대 부하량
  * **시나리오 기대 부하:** 위 기준 시나리오를 적용했을 때 각 노드에 실제로 가해지는 부하량
  * 두 지표를 비교하여 시스템의 안정성과 여유 용량을 확인합니다.

### 5. 노드별 성능 및 수용량 매트릭스

아래 표는 각 노드의 **이론적 최대 성능**을 바탕으로, 해당 노드 단독으로 감당할 수 있는 최대 동시 통화 수와 가입자 수를 역산한 결과입니다. 이를 통해 **LLM이 전체 시스템의 병목 구간**임을 확인할 수 있습니다.
...
* **시나리오 기대 부하:** **§6.4에서 잡은 트래픽 규모**(예: 동시 전체 통화 **428**호, 그중 AI **약 85**호, CPS **약 3.56** 등)가 **실제로 걸렸다고 가정할 때**, 그 순간 **해당 노드 한 줄**에 들어오는 부하량이다. 노드마다 단위가 다르다(LLM이면 **QPS**, STT면 **동시 스트림 수**, API면 **동시 WSS** 등). **「그 시나리오를 감당하려면 이 노드에 얼마나 걸리나」**를 한눈에 보기 위한 칸이며, **벤치 상한보다 낮을수록** 그 노드는 그 시나리오에서 **여유**가 있다는 뜻이다(단, 시스템 전체 한도는 **가장 먼저 찬 병목 노드**가 정한다).
* **최대 수용 가능 전체 동시호:** 다른 노드의 성능은 무한하다고 가정할 때, **오직 해당 노드의 성능 한계만으로** 감당할 수 있는 시스템 전체의 최대 동시 통화 수입니다. (이 수치가 가장 낮은 'LLM(428호)'이 전체 시스템의 실제 수용 한계가 됩니다.)
...
| 시스템 노드 | 성능 측정 단위 | 이론적 최대 성능 (벤치마크) | 시나리오 기대 부하 | 최대 수용 가능 전체 동시호 | 최대 수용 가능 가입자 (시간당) | 구성 대수 |
|-------------|----------------|-----------------------------|--------------------|----------------------------|--------------------------------|-----------|
| **LLM** | 추론 QPS | 25 (운영 상한 20) | 20 | **428호** (AI 85호) | **12,840명** | 2 |
| **AI Runtime** | 동시 세션 | 500 | 85 | 2,500호 | 75,000명 | 2 |
| **STT** | 동시 스트림 | 1,250 | 약 770 (428호×1.8) | **694호** (호당 **1.8**스트림 가정) | 20,820명 | 2 |
| **TTS** | 동시 합성 | 2,000 | 85 이하 (피크 기준) | 10,000호 | 300,000명 | 2 |
| **API/Realtime**| 동시 WSS | 5,000 (Active 1대 벤치 가정, §5.5) | **약 428** (1통화·1WSS, §6.4 전체 동시호) | **5,000호** (통화·WSS 1:1이면 벤치와 동일) | **150,000명** (5,000×30, 1콜/h·120초) | 1+1 |
| **AIR GW** | 시그널 TPS | 제품별 벤치마크 필요 | 약 3.56 CPS + 이벤트 | - | - | 1+1 |
| **Qdrant** | 검색 QPS | 약 200 | 호당 0.2 QPS 가정 시 85 | 5,000호 | 150,000명 | 2 |
| **PostgreSQL** | TPS / 커넥션 | 벤치마크 필요 | 시나리오 트랜잭션 환산 | - | - | 2 |

*(STT: 벤치는 **스트림/채널** 단위. **770 ≈ 428×1.8** 은 §6.4 동시 전체 호에 대한 기대 스트림 수.)*

### 6. 도입 비용 요약

| 구분 | 예상 비용 (ROM) | 비고 및 상세 참조 |
|------|-----------------|-------------------|
| **하드웨어 인프라 (CAPEX)** | **약 2.7억 ~ 4.0억 원** | AI Call Agent 16노드 기준 (11.2절) |
...
- **동시세션(운영 기준)**: 428호 (전체 호), 85호 (AI 호) (Busy Hour 기준)
- **CPS**: 약 3.56 INVITE/s 지속 유입
- **AI 적용률 가정**: 20% (전체 호 중 20%만 AI 응대)
- **실시간 AI 활성 구간 가정**:
  - STT 활성 세션: `~770` (전체 호 대상 실시간 CID 및 AI 응대)
  - TTS 동시 합성 기준: 서버 처리량 기준으로 산정(`1,000 세션/노드`, 1 Active = 1,000)
  - LLM 동시 생성 요청: §6.4 계산식 기준(발화 빈도·질의수 및 유저간 통화 종료 요약 가정)
...
- **처리량 가정(벤치)**: **625 동시 스트림/노드** — 여기서 **스트림 1개 = RTP 미러(또는 동등)로 STT에 연결된 연속 ASR 디코딩 채널 1개**이며, **통화 1건과 1:1이 아닐 수 있다**(§4·§6.4 호당 스트림 가정 참조).
- **구성**: 본 시나리오 **2노드 All-Active** → 클러스터 **1,250 동시 스트림**(벤치 합산). §6.4 기준 기대 **약 770 스트림**(428호×1.8) 대비 여유.
...
- **구성**: 본 시나리오 **2노드 All-Active** → 클러스터 **2,000 동시**(벤치 합산). (§6.4 AI 호 기준 합성 부하 **≤85** 대비 여유)
...
- **QPS 및 동시 call 산정 (클러스터 2노드)**
  ...
  - 운영 동시 call(권장): `20 / 0.0467 = 428` → **428 전체 call (AI 85호)**
  - 본 문서의 기준 용량은 **운영 기준 428 동시 전체 세션(AI 85호)** 으로 둔다.
...
- **시나리오 부하 (§6.4):** 코어 **~3.56 CPS** 및 **인바운드 rps**는 트리거 빈도에 따라 환산한다. **통화 1건당 장기 WSS 1건(1:1)** 이면 동시 WSS는 **동시 통화 수와 동일**이며, 본 §6.4 기준 전체 **428**호일 때 **약 428** WSS(벤치 5,000 대비 여유). 호마다 WSS를 나누거나 집계 채널만 쓰는 모델이면 별도 환산한다. 빠른 참조 **매트릭스·§6.2**와 맞춘다.
...
**제목의「250」:** 과거 문서 버전에서 LLM 병목으로 쓰이던 **동시 AI 통화 목표치** 예시이며, 현재 §6.4는 **약 85 AI 호·428 전체 호** 기준으로 읽는다. **모든 노드의 벤치 단위를 250으로 맞추는 값은 아니다**(빠른 참조 매트릭스·§6.2 참조).
...
| AI Call Agent 시스템 | AIR 연동 접점 GW | **~3.56** CPS권 시그널 + **85** 동시 호 컨텍스트 이벤트 | 제품·프로파일 **TPS 벤치** | 1 | 1 Active + 1 Standby |
| AI Call Agent 시스템 | STT | **~770** 동시 스트림(유저간 2, AI 1) | **625**/노드(벤치) | **2** | **2 Active** (All-Active; 클러스터 **1,250** 동시) |
| AI Call Agent 시스템 | TTS | **≤85** 동시 합성(턴 피크, 보수) | **1,000**/노드(벤치) | **2** | **2 Active** (All-Active; 클러스터 **2,000**) |
| AI Call Agent 시스템 | LLM | **20** QPS (80% 운영) | `1,500 tok/s x 2노드`, 120 tok/req → **25** QPS 벤치 | 2 | **2 Active (fp8)** |
| AI Call Agent 시스템 | AI Runtime | **85** 동시 세션(오케스트레이션) | **250**/노드(벤치) | **2** | **2 Active** (All-Active; GW 해시; 클러스터 **500** 동시 벤치) |
| AI Call Agent 시스템 | API/Realtime | **약 428** 동시 WSS(통화·WSS **1:1**, §6.4 전체 동시호) + 코어 **~3.56 CPS** 기반 **rps** | **5,000 WS** (가정, §5.5 참조) | 1 | **1 Active + 1 Standby** · 단일 VIP §1.6 |
...
> 상기 수치는 초기 계획치이며, 반드시 스테이징에서 **3.56 CPS/120초/428 동시세션 (AI 85)** 부하로 재검증한다.
...
- 시나리오 가정:
  ...
  - **LLM 사용:** AI 호는 발화당 1회(호당 24회), 유저간 통화는 종료 시 1회(호당 1회)
  - **STT 사용:** AI 호 및 유저간 통화 모두 실시간 CID용으로 상시 사용. **호당 동시 ASR 스트림 수**는 §4와 같이 **유저간 2·AI 1**을 가정하면 평균 **0.8×2 + 0.2×1 = 1.8 스트림/호**이며, 동시 전체 호 **N**일 때 기대 스트림은 **약 1.8×N**이다(428호면 **≈770**).
  - **TTS 사용:** AI 호에서만 사용
  - 유저는 `1시간에 1회 통화`
- LLM 2노드(fp8 1,500 tok/s/노드), 120 tok/req 기준:
  - 이론 QPS: `(1,500 x 2) / 120 = 25 req/s`
  - 운영 QPS(80%): `20 req/s`
  - 호 1개당 평균 LLM QPS: `(0.2 * 24회 + 0.8 * 1회) / 120초 = 5.6 / 120 ≈ 0.0467 QPS/호`
  - **최대 수용 전체 동시호 (LLM 병목 시):** `20 QPS / 0.0467 QPS/호 ≈ 428 동시호`
- 전체 통화 및 가입자 환산 (AI 비율 20% 적용):
  - **최대 수용 AI 동시호:** `428 * 0.2 ≈ 85 동시호`
  - 시간당 처리 가능한 전체 통화 수 (CPS 환산): `428 동시호 x (3600 / 120) = 12,840 콜/시간` (약 **3.56 CPS**)
  - **시간당 수용 가능한 가입자 수(유저당 1콜/시간 가정): `12,840명/시간`**
...
- **시나리오 축:** §6.4 가상 시나리오에서 **동시 전체 통화 428호 (AI 통화 85호)** 은 **LLM 병목 역산 결과**이며, STT·AIR 등은 그에 따른 **파생 부하**다.
- **LLM:** 시나리오 **20 QPS** / 벤치 **25 QPS** → 운영 **80%** 규칙 내 여유.
- **STT:** 시나리오 **~770** 동시 스트림 / 벤치 클러스터 **1,250** → 여유.
- **TTS:** 시나리오 **≤85** 동시 합성(보수) / 벤치 **2,000** → 큰 여유(피크는 부하로 재확인).
- **AI Runtime:** 시나리오 **85** 세션 / 벤치 클러스터 **500** → 한 대 장애 시에도 단기 이월 가능(해시 재균형).
- **API/Realtime:** **통화·WSS 1:1** 이면 §6.4 전체 **428**호에 **동시 WSS 약 428**; 단독 병목이 될 때의 상한은 **벤치 5,000 WSS = 동시 통화 5,000호**(Active 1대 기준). 벤치는 **§5.5**에서 확정.
