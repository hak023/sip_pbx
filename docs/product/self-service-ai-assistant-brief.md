...
**작성일**: 2026-07-14
**버전**: 0.4 (2026-07-20 갱신 — Phase 2 범위 추가: 설정 카탈로그/Screen Graph의 하드코딩 의존도를 낮추기 위해 동적 구성(DB 기반 + 프론트엔드 다운로드/업로드)로 전환하고, IntelliDecision의 키워드 정규식 힌트 의존을 제거하는 개선 범위를 명시)
...
2. **핵심 소스: 설정 카탈로그(Settings Catalog) + Tool-calling**
   ...
   | 5   | **통화 이력 자연어 질의(Call History NLQ)**        | 자기 번호(owner)로 걸려온/걸었던 통화 이력 전체(요약·발신번호·통화량·미응답 여부) | **Tool-calling 기반 구조화 검색/집계**(키워드 검색·Top-N 집계·오늘자 미응답 조회, 새 임베딩 파이프라인 없이 기존 `call_records` 재사용) |
...
- **통화 이력 자연어 질의(Call History NLQ, 2026-07-20 범위 추가)**: 자기 번호(owner)의 통화 이력(`call-history` API가 참조하는 `call_records` 데이터)에 AI가 접근하여 다음 3가지 질의 유형에 응답한다 — (1) 특정 키워드로 대화한 통화 검색, (2) 특정 기간 내 최다 발신 번호 집계, (3) 오늘자 미응답(수신 못한) 번호 조회. 새로운 벡터 임베딩 파이프라인을 구축하지 않고, 기존 `call_records`(caller_id/call_summary/has_recording 등)를 구조화 검색·집계하는 Tool-calling으로 구현한다(§Technical Considerations 참고).
...
- 통화 이력 NLQ 고도화(복합 조건 질의 — 예: "이번 주 예약 문의 중 미해결 건만", 그래프성 요약 음성 안내). 2026-07-20 기준 기본 3종 질의(키워드 검색/Top-N 집계/오늘자 미응답)는 MVP로 승격되었으므로, 여기서는 그 이상의 고도화만 남긴다.
...
## Phase 2: 설정 카탈로그/Screen Graph 동적화 및 IntelliDecision 신뢰성 개선 (2026-07-20 추가)

### 문제 인식

Epic 1 구현을 마친 후 실운용해 보니, 다음 3가지 구조적 한계가 드러났다.

1. **설정 카탈로그/Screen Graph가 순수 Python 하드코딩 레지스트리다**(`settings_catalog.py::_CATALOG`,
   `screen_graph.py::_SCREEN_REGISTRY`). 프론트엔드에 새 설정 필드나 화면이 추가될 때마다 백엔드
   Python 코드를 직접 수정·배포해야 만 AI가 이를 인지한다(FR11이 이미 "카탈로그 등록 누락 위험"을
   경고했으나, 근본적으로는 "등록 자체가 코드 배포 필요"라는 구조적 문제가 남아 있었다).
2. **IntelliDecision(Story 1.10)이 정규식 키워드 패턴(`intent_tier.py`)으로 힌트를 산출한다**. 문서상으로는
   "참고 신호일 뿐 최종 판단은 항상 LLM"이라고 명시되어 있지만, 음성 STT 결과는 종결 어미·조사가
   까질리거나 누락되는 일이 흔해, 키워드 매칭이 실제와 정반대로 잘못된 힌트를 생성해 LLM을
   은근히 잘못된 방향으로 유도할 위험이 있다(참고용이라 해도 "잘못된 참고"는 위험 자체다).
3. **카탈로그/Screen Graph/매뉴얼 섹션-도메인 매핑이 모두 "지식베이스나 DB가 아닌 Python 모듈 내
   상수"로만 존재해**, 실제로 "동적으로 구성된다"고 부를 수 있는지에 대한 검증이 없었다(해당 데이터가
   실제로 서버 재시작 없이 갱신 가능한 저장소에 있는지 기능 체크 필요).

### 목표

- 신규 설정 필드/화면이 생기면 **별도 백엔드 코드 배포 없이**(이미 등록된 조회/변경 함수를 재사용하는 한)
  프론트엔드에서 메타데이터를 다운로드·수정·업로드하여 AI가 즉시 인식하도록 한다.
- IntelliDecision은 키워드 정규식에 의존하지 않고 LLM 판단만으로 동작해야 한다(`.github/copilot-instructions.md`의
  "의도 분류는 키워드 매칭보다 LLM 판단을 우선한다" 원칙을 이번 개선으로 완전히 충족).
- 카탈로그/Screen Graph 설정은 실제 DB(또는 지식베이스) 기반으로 저장되고, 업로드 즉시 반영(핵 리로드)되며,
  버전 이력·롤백이 가능해야 한다.

### 명시적 범위 외(중요 제약)

- **완전 노코드는 아니다**: `get_fn`/`update_fn`이 호출하는 실제 서비스 함수(예:
  `persona_service.save_persona`)는 여전히 코드로 존재해야 한다. 완전히 새로운 도메인(예: 지금까지 없던 설정
  항목)은 메타데이터 업로드만으로는 지원되지 않으며, 해당 조회/변경 함수가 먼저 코드로
  등록(함수 레지스트리에 추가)되어 있어야 한다. 즉, 동적화는 "이미 존재하는 함수의 메타데이터
  노출 범위를 코드 변경 없이 조정"하는 것이지, "임의 함수를 데이터만으로 생성"하는 것이 아니다.
