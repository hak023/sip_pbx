...
**작성일**: 2026-07-14
**버전**: 1.8 (2026-08-05 갱신 — FR34-F 세션 그룹핑 키 수정: call_id 단일 기준 → 채널별(음성/채팅) 그룹핑)
...
| IntelliDecision 정책 레지스트리·다중 홉 확장              | 2026-07-28 | 0.9     | IntelliDecision 지식 구조화 리서치(축 A/B) 승인 즉시 구현(FR28, Story 1.18) — `intellidecision_policy.py`(유형 데이터화)/`knowledge_graph.py`(Screen Graph 2-hop, 쓰기 가능 여부 기반 유형 필터링을 프롬프트에 명시 주입)                                                                                                                                                                                                                                                                              | Copilot (BMAD PM 역할)  |
| IntelliDecision 프롬프트 산문 자동 렌더링(축 A 완전판)    | 2026-07-28 | 1.0     | 기존 하드코딩된 번호 목록을 `prompt_rules.py` 데이터 기반 자동 렌더링으로 전환(FR29, Story 1.19) — 프롬프트 번호 재조정 함정의 근본 해결                                                                                                                                                                                                                                                                                                                                                               | Copilot (BMAD PM 역할)  |
| IntelliDecision 판단 근거 투명성                          | 2026-07-29 | 1.1     | 외부 레퍼런스 리서치([SELF_SERVICE_CORE_FEATURES_EXTERNAL_RESEARCH.md](../design/SELF_SERVICE_CORE_FEATURES_EXTERNAL_RESEARCH.md))에서 제안한 "판단 근거 로깅" 개선을 사용자가 승인·확장(프론트엔드 투명성 노출 요구 추가) — FR30/NFR6 신설, Story 1.20(설계 결정)~1.22(프론트엔드 열람) 추가                                                                                                                                                                                                          | Copilot (BMAD PM 역할)  |
| RAG·IntelliDecision 고도화(벤더 비종속·투명성·사전예측)   | 2026-07-30 | 1.2     | 사용자가 현재 구현 규모에 국한하지 않는 유연한 데이터 수집 + 지식베이스 구성 투명성 + IntelliDecision 분기별 RAG 매칭 사전예측을 신규 요청 → 시장·연구 리서치([SELF_SERVICE_RAG_INTELLIDECISION_ADVANCEMENT_RESEARCH.md](../design/SELF_SERVICE_RAG_INTELLIDECISION_ADVANCEMENT_RESEARCH.md)) 승인 반영 — FR31 신설, 우선순위 ②(지식베이스 인벤토리 투명성) → ③(RAG 매칭 정책 레지스트리 확장) → ①(소스 어댑터 벤더 비종속화) 순으로 Story 1.23부터 착수                                               | Copilot (BMAD PM 역할)  |
| 도메인 비종속 지식베이스 플랫폼 확장                      | 2026-08-04 | 1.3     | 사용자가 FR31 범위를 넘어서는 더 큰 전환을 요청(특정 도메인에 묶이지 않는 범용 지식베이스, 웹 업로드·CRUD, PDF/OpenAPI 포함 소스 어댑터, n-hop 그래프 구조, 실제 LLM 응답까지 생성하는 응답 시뮬레이터) → 리서치 문서 재작성([SELF_SERVICE_RAG_INTELLIDECISION_ADVANCEMENT_RESEARCH.md](../design/SELF_SERVICE_RAG_INTELLIDECISION_ADVANCEMENT_RESEARCH.md) v2.0) — FR32 신설, Story 1.26(CRUD+업로드)→1.27(응답 시뮬레이터)→1.28(그래프 n-hop 일반화)→1.29(Contextual Retrieval 스파이크) 순으로 착수 | Copilot (BMAD PM 역할)  |
| Story 1.26 구현 완료                                      | 2026-08-04 | 1.3     | FR32-A 구현 완료(Review) — 지식 문서 CRUD API(`/api/knowledge-base/documents`), `PdfDocumentAdapter`/`OpenApiSpecAdapter`, `knowledge_documents` DB 테이블, 프론트엔드 업로드 탭. 신규 단위테스트 27건 통과, 기존 self_service 회귀 무영향. 실서버 IV만 잔여                                                                                                                                                                                                                                           | Copilot (BMAD Dev 역할) |
| Story 1.26 실서버 IV 검증 완료                            | 2026-08-04 | 1.3     | 사용자가 서버 재시작 후 업로드→조회→수정→삭제 전체 사이클을 실제 API 호출로 검증, `logs/app.log` 원시 로그 cross-check 일치 확인, 브라우저로 프론트엔드 "지식 업로드" 탭 렌더링 확인. Story 1.26 Status Review→Done 전환                                                                                                                                                                                                                                                                               | Copilot (BMAD QA 역할)  |  | FR33 신설 — 업로드 통합·지식베이스 자동 구성·시뮬레이터 UX 고도화 | 2026-08-04 | 1.4 | 사용자가 FR32 완료 후 잔여 정적 결합(설정 관리/지식 업로드 탭 이원화, 도메인 종속 카탈로그, 시뮬레이터 UX)을 지적하며 계획 수립 요청 → FR33-A(업로드 진입점 통합)~E(유형 C 하이브리드 RAG) 신설, Story 1.30~1.33(Draft) 신규 작성, NFR8 추가 | Copilot (BMAD PM 역할) | --- |  | FR34 신설 — 도메인 비종속 AI 에이전트 플랫폼 전환(Epic 3) | 2026-08-05 | 1.5 | 사용자가 시장·학계 리서치([MCP_VS_CLIENT_CENTRIC_UNIVERSAL_AGENT_MARKET_RESEARCH.md](../design/MCP_VS_CLIENT_CENTRIC_UNIVERSAL_AGENT_MARKET_RESEARCH.md)) 기반으로 개발 방향 재편 요청(실제 API 실행 지원/Frontend IA 재편/지식베이스·IntelliDecision 투명성 고도화/시뮬레이터 통합 재검토) → FR34-A~E 신설, NFR9 추가, Story 1.34~1.39(Draft) 신규 작성 | Copilot (BMAD PM 역할) |
| FR34 수정 — IntelliDecision 탭·시뮬레이터·판단이력 재설계 | 2026-08-05 | 1.6     | 사용자가 생상(1)"AI 의사결정 로직" 탭을 시뮬레이션이 아니라 지식베이스 기반 설명 매뉴얼로 (2)응답 시뮬레이터를 삭제하고 기존 숨겨진 SIP MESSAGE 패널(`GlobalSmsDock`)을 상시 활용해 자기 자신에게 실제 문자를 보내는 방식으로 (3)"최근 판단 이력"을 실제 대화 기반 순서도로 고도화하라는 추가 요청 → FR34-D/E 수정(시뮬레이션 방식 폐기), FR34-F 신설, NFR10 추가, Story 1.38(범위 재정의)/1.39(확정 결정)/1.40(신규 작성) 갱신                                                                        | Copilot (BMAD PM 역할)  |
...
- **FR28 (IntelliDecision 정책 레지스트리 + Screen Graph 다중 홉 확장, 2026-07-28 범위 추가, Story 1.18)**: 리서치(`docs/design/SELF_SERVICE_INTELLIDECISION_KNOWLEDGE_STRUCTURING_RESEARCH.md`) 축 A/B를 반영한다.
  - 유형 A~I(FR12/FR25/FR26)의 코드/이름/트리거 예시/Tool 필요 여부/쓰기 가능 도메인 전제 여부를 `intellidecision_policy.py`의 정적 레지스트리로 데이터화해, 코드·향후 시각화(축 C)가 조회 가능하도록 해야 한다. 기존 검증된 프롬프트 산문(FR12/25/26)은 회귀 위험 최소화를 위해 이번 범위에서 그대로 유지한다.
  - Screen Graph(FR13, Story 1.11)의 1-hop 팬아웃(매뉴얼→도메인→화면)을 `knowledge_graph.py`의 `traverse()`로 2-hop(도메인→쓰기 가능 여부→적용 가능 IntelliDecision 유형)까지 확장해야 한다.
  - 2-hop 결과(쓰기 가능 여부)는 시스템 프롬프트의 화면 안내 섹션에 명시적 힌트로 주입되어, LLM이 쓰기 불가능한 도메인에서 유형 B/D/E/G(변경·되돌리기)를 잘못 안내하는 환각을 구조적으로 줄여야 한다(Anthropic "투명성" 원칙 — 판단 근거를 프롬프트에 직접 드러냄).
  - 그래프 DB·LLM 자동 엔터티 추출·Full GraphRAG 패키지 도입은 Non-Goal로 유지한다(규모가 작고 관계가 이미 기지, 리서치 §3.3/§5 근거).
- **FR29 (IntelliDecision 프롬프트 산문 자동 렌더링, 2026-07-28 범위 추가, Story 1.19, 축 A 완전판)**: FR28이 의도적으로 보류한 프롬프트 산문의 완전한 데이터 기반 렌더링을 반영한다.
  - 기존 18개 응답 규칙(일반 규칙+유형 A~I 관련 규칙)은 `prompt_rules.py`의 정적 리스트(`_BASE_RULES`/`_TOOL_RULES`)로 이관되고, 번호는 등록 순서에서 자동 계산되어야 한다(사람이 번호를 직접 세지 않음).
  - 규칙 간 번호 교차 참조(예: "유형 C(7번)")는 렌더링 시점에 실제 번호로 자동 치환되어야 하며, 그 결과에 센티넬 토큰이 남아있으면 안 된다.
  - 외부 template placeholder(`{fallback_message}` 등)는 렌더링 과정에서 훼손되지 않고 그대로 보존되어야 한다.
  - 기존 응대 품질(응답 내용)은 회귀 없이 그대로 유지되어야 한다(서식·들여쓰기 차이는 허용되나, 의미 변경은 허용되지 않음).
- **FR30 (IntelliDecision 판단 근거 투명성, 2026-07-29 범위 추가, Story 1.20~1.22)**: 사용자가 "판단 근거 로깅은 투명성에 근거한 기능이므로 프론트엔드까지 반영해 확인 가능해야 한다"고 요청함에 따라, 시스템은 각 셀프서비스 응대 턴에서 **IntelliDecision이 어떤 유형(A~I)으로 판단했고 왜 그렇게 판단했는지**를 구조화된 형태로 캡처·저장·열람 가능하게 해야 한다.
  - 판단 결과(유형 코드, 판단 근거 요약, 관련 도메인/화면)는 매 턴 `call_data_record` 로그와 별도의 조회 가능한 저장소(신규 테이블 또는 기존 변경 이력 테이블 확장)에 기록되어야 한다.
  - 판단 근거 캡처가 **기존 응답 생성 LLM 호출에 추가 왕복(round-trip)을 요구하지 않아야 한다**(NFR1 지연 예산 보호) — 기존 호출의 구조화 출력/Tool 부가 필드로 동시 캡처하는 방식을 우선 검토한다(구체적 방식은 Story 1.20에서 스파이크로 결정).
  - 캡처 시도 실패(파싱 실패 등) 시에도 **사용자에게 보여줄 실제 응답 생성 흐름에는 영향을 주지 않아야 한다**(판단 근거는 어디까지나 부가 정보이며, 실패 시 `unknown`으로 기록하고 넘어간다 — 회귀 방지 최우선).
  - 프론트엔드(`settings/ai-assistant/*`)에 테넌트 관리자가 **최근 판단 근거 이력을 읽기 전용으로 열람**할 수 있는 화면(신규 페이지 또는 기존 "설정 변경 이력" 페이지 확장)이 추가되어야 한다 — 유형 코드·판단 근거 요약·발화 시각·(있다면) 관련 화면 안내를 표시한다.
  - 개인정보/민감정보 노출 최소화를 위해 원본 발화 전문이 아니라 판단에 필요한 최소 요약만 저장·표시해야 한다.
- **FR31 (RAG·IntelliDecision 고도화 — 벤더 비종속 수집·지식베이스 투명성·분기별 매칭 사전예측, 2026-07-30 범위 추가, Story 1.23~)**: 사용자 요청([SELF_SERVICE_RAG_INTELLIDECISION_ADVANCEMENT_RESEARCH.md](../design/SELF_SERVICE_RAG_INTELLIDECISION_ADVANCEMENT_RESEARCH.md) 리서치 반영)에 따라 시스템은 아래 3개 하위 요구사항을 충족해야 한다. 우선순위는 ②→③→①이며, 각각 독립적으로 완료 가능하도록 Story를 분리한다.
  - **FR31-A (지식베이스 구성 투명성, 우선순위 1)**: 시스템은 매뉴얼 RAG(ChromaDB) 색인 현황을 읽기 전용으로 조회할 수 있는 API(예: 컬렉션별 문서 수, 도메인 분포, 최근 색인 시각, 소스 유형)를 제공해야 하며, 프론트엔드(`settings/ai-assistant/docs`)에 기존 「AI 의사결정 로직」 탭과 나란히 「지식베이스 현황」 탭을 추가해 이를 열람 가능하게 해야 한다. 응대 로직 자체에는 영향을 주지 않는 순수 관측 기능이다.
  - **FR31-B (IntelliDecision 분기별 RAG 매칭 사전예측, 우선순위 2)**: `intellidecision_policy.py`의 유형 A~I 레지스트리 항목에 RAG 매칭 메타데이터(`rag_enabled`/`rag_source_scope`/`rag_strategy_hint` 등)를 추가하고, 기존 `GET /intellidecision-policy` 응답에 함께 노출해 「이 유형의 발화가 어떤 지식 소스·전략으로 매칭될 예정인지」를 대화 실행 전에도 표/그래프에서 확인 가능하게 해야 한다. 아울러 실제 RAG 실행 시점에 검색 쿼리·top-K 문서·점수를 `logs/app.log`에 trace로 남겨, 사전 예측(정책 레지스트리 표시)과 사후 실측(trace 로그)을 대조 검증할 수 있어야 한다.
  - **FR31-C (벤더 비종속·유연한 데이터 수집, 우선순위 3)**: `manual_indexer.py`(현재 마크다운 전용 파서)를 소스 어댑터 인터페이스로 일반화해, 신규 소스 유형(API/Tool 문서 등)을 어댑터 추가만으로 색인 가능하게 해야 한다. 색인 파이프라인에는 청크별 자동 맥락 요약(Contextual Retrieval 방식) 및 API/Tool류 데이터에 유리한 BM25 하이브리드 검색을 공통 후처리 단계로 검토·반영한다. 특정 임베딩 벤더·벡터DB(ChromaDB)에 대한 하드 의존은 이번 범위에서 완전히 제거하지 않되, 소스 어댑터 계층만큼은 벤더/스키마 비의존적으로 설계해야 한다.
  - Full GraphRAG·독립 벡터DB 네임스페이스 물리 격리 등 인프라 복잡도가 큰 옵션은 이번에도 Non-Goal로 유지한다(리서치 §0/§3.4 근거 — 현재 규모에서는 과설계).
- **FR32 (도메인 비종속 지식베이스 플랫폼, 2026-08-04 범위 추가, Story 1.26~1.29)**: 사용자 요청([SELF_SERVICE_RAG_INTELLIDECISION_ADVANCEMENT_RESEARCH.md](../design/SELF_SERVICE_RAG_INTELLIDECISION_ADVANCEMENT_RESEARCH.md) v2.0 반영)에 따라 시스템은 셀프서비스 매뉴얼에 국한되지 않는 범용 지식베이스로 확장되어야 한다.
  - **FR32-A (웹 업로드·CRUD, 우선순위 1, Story 1.26)**: 시스템은 프론트엔드에서 마크다운/일반 텍스트/PDF/OpenAPI 스펙 문서를 업로드해 지식베이스에 등록하고, 등록된 문서를 조회·수정·삭제할 수 있는 API 및 화면을 제공해야 한다. 업로드 문서는 `owner`/`domain_tags`(자유 텍스트 태그)/`source_type`/`version` 메타데이터를 가져야 하며, `domain_tags`는 기존 `settings_catalog.py`의 고정 7개 도메인에 종속되지 않아야 한다(목표 ①). 기존 매뉴얼 색인 방식(태그 파싱)과 `owner` 테넌트 격리는 그대로 보존해야 한다.
  - **FR32-B (응답 시뮬레이터, 우선순위 2, Story 1.27)**: 시스템은 운영자가 예시 질문을 입력하면 실제 통화/채팅 세션에 영향을 주지 않는 격리된 테스트 경로로 (1) 매칭된 지식 문서/청크, (2) 판정된 IntelliDecision 유형과 판단 근거, (3) 실제 LLM 호출로 생성한 최종 응답 텍스트를 함께 반환해야 한다 — 대화를 실제로 걸어보지 않고도 특정 질문에 대한 응답을 사전에 확인할 수 있어야 한다(목표 ⑤⑥).
  - **FR32-C (지식 그래프 n-hop 일반화, 우선순위 3, Story 1.28)**: `knowledge_graph.py`의 고정 3단 체인(`manual_qa→catalog_domain→frontend_screen→intent_type`)을 노드/엣지 타입을 등록 가능한 구조로 일반화하고, `document`/`api_endpoint`/`procedure_step` 노드 타입을 예약해야 한다. `max_hops` 파라미터는 실제로 3~4단계까지 가변적으로 동작해야 한다(목표 ④).
  - **FR32-D (Contextual Retrieval 스파이크, 우선순위 4, Story 1.29, 실측 완료 2026-08-04)**: 청크별 자동 맥락 요약 적용 여부는 비용·품질 실측 스파이크로 결정한다 — **실측 결론: 미채택**(owner 9001 매뉴얼 Q&A 52건 self-retrieval 벤치마크에서 baseline 대비 hit@1이 94.23%→86.54%로 오히려 악화, 52회 LLM 호출·362.9초 비용만 추가). 순수 파이썬 BM25(외부 의존성 없음)는 동일 벤치마크에서 hit@1/hit@3 100%로 벡터 검색을 앞질러, 후속 Story에서 실사용 발화 기준 재검증 후 조건부 채택을 검토한다(결정 리포트: `docs/reports/2026-08/2026-08-04_story_1.29_contextual_retrieval_spike_decision.md`).
  - Full GraphRAG 자동 엔터티 추출, ChromaDB 이외 벡터DB 마이그레이션, 독립 벡터DB 네임스페이스 물리 격리는 이번에도 Non-Goal로 유지한다.
- **FR33 (업로드 통합·지식베이스 자동 구성·응답 시뮬레이터 고도화, 2026-08-04 범위 추가, Story 1.30~1.33)**: 사용자 요청(FR32 완료 후 발견된 잔여 정적 결합 + 셀프서비스 AI를 "이 시스템 전용"에서 "업로드된 상세 데이터만으로 임의 시스템에 응대 가능한 플랫폼"으로 전환)에 따라 시스템은 아래 5개 하위 요구사항을 충족해야 한다. 우선순위는 ①업로드 진입점 통합(회귀 위험 최소·즉시 체감) → ②지식베이스 자동 구성(플랫폼 전환의 핵심) → ③공통 고정 콘텐츠 화이트리스트 → ④응답 시뮬레이터/IntelliDecision UX 고도화 → ⑤유형 C 하이브리드 RAG 순서다.
  - **FR33-A (업로드 진입점 통합, 우선순위 1, Story 1.30)**: `settings/ai-assistant/docs` 페이지의 "지식 업로드" 탭(Story 1.26, 테넌트별 매뉴얼/PDF/OpenAPI)과 "설정 관리" 탭(Epic 2 Story 2.4/2.5, `catalog_config`/`screen_graph` JSON export/import)은 서로 다른 저장소(`knowledge_documents` 테이블 vs `self_service_catalog_config` 테이블)와 서로 다른 목적(테넌트별 지식 vs 시스템 공통 설정 스키마)을 가지므로 **데이터 모델을 강제로 통합하지 않는다**. 대신 사용자 경험상 진입점을 하나로 통합한다 — 단일 "지식 업로드" 탭 안에 소스 유형 선택 단계를 두어 "① 테넌트 지식 문서(매뉴얼/PDF/OpenAPI 등, `knowledge_documents`)"와 "② 시스템 공통 설정/구성(`catalog_config`/`screen_graph`, FR33-C의 고정 콘텐츠)"을 같은 화면에서 업로드·조회할 수 있게 하고, 기존 "설정 관리" 탭은 이 통합 탭의 하위 섹션으로 흡수한다(별도 최상위 탭 제거).
  - **FR33-B (업로드 데이터 기반 지식베이스 자동 구성, 우선순위 2, Story 1.31)**: 시스템은 업로드된 상세 데이터(API/Tool 정보, 매뉴얼 텍스트, 화면 설명 등)만으로 3종 지식베이스 구성요소 — (1) 이용 매뉴얼 Q&A, (2) AI가 변경 가능한 설정 목록(writable fields), (3) 화면 안내(Screen Graph 노드) — 를 **자동으로 생성**해야 한다. 이는 현재 시스템(SIP PBX 셀프서비스)에 종속된 하드코딩 카탈로그/Screen Graph 레지스트리에 대한 지식 구조화가 아니라, **임의의 다른 시스템이라도 동일한 형식(API 스펙+매뉴얼+화면 설명)의 데이터를 업로드하면 해당 테넌트 전용 지식베이스가 구성되어야 한다**는 뜻이다. 실제 설정 변경 Tool 호출(get_fn/update_fn 실행부)이 업로드 데이터와 정확히 매칭되어 동작하는지는 이번 범위에서 보장하지 않는다(참고사항에 따라 추후 과제로 명시적으로 이연) — 이번 범위는 업로드 데이터 기반 지식베이스 자동 구성과 그 지식을 활용한 Q&A/화면 안내 응대까지다.
  - **FR33-C (테넌트/도메인 공통 고정 콘텐츠, 우선순위 3, Story 1.30에 통합 구현)**: `intellidecision_policy.py`의 유형 A~I 정책, 공통 대화 예절/안전 규칙 등 테넌트·도메인에 무관하게 재사용 가능한 콘텐츠는 시스템 표준(고정) 지식으로 분류해 테넌트별 업로드 없이도 기본 제공되어야 한다. FR33-A의 업로드 통합 탭에서 이 고정 콘텐츠는 "시스템 표준"으로 구분 표시하고, 원칙적으로 읽기 전용이되(운영자 권한으로 조정 가능한 항목은 FR33-A의 "② 시스템 공통 설정/구성" 섹션에서 편집), 테넌트별 업로드 지식과 시각적으로 명확히 구분되어야 한다.
  - **FR33-D (응답 시뮬레이터·IntelliDecision UX 통합 고도화, 우선순위 4, Story 1.32)**: 기존 "IntelliDecision 정책" 탭(Story 1.18)과 "응답 시뮬레이터" 탭(Story 1.27)을 하나의 통합 화면으로 재구성한다. 상세 요구사항:
    1. 유형 A~I 각각을 별도 tab으로 제공한다.
    2. 각 tab에는 해당 유형에 대응하는 질문 예시가 여러 개 리스트로 제공되며 운영자가 선택할 수 있다. 예시 목록은 정적 하드코딩이 아니라 지식베이스(FR33-B로 자동 구성된 매뉴얼 Q&A/화면 안내)에서 유형별로 동적 추출되어야 한다.
    3. 각 질문 예시 옆에는 그 질문이 RAG 매칭이 필요한지/Tool 호출이 필요한지 등 정책 메타데이터(FR31-B의 `rag_enabled`/`rag_source_scope`/`rag_strategy_hint`, Story 1.24)가 지식베이스 기반으로 표시되어야 한다.
    4. 질문을 선택하면 응답 시뮬레이터(Story 1.27 API 재사용)가 실행되어 (a) 판정된 유형과 관련 유형 연결, (b) RAG 매칭 근거(문서/청크/hop 경로), (c) `knowledge_graph.traverse()`의 n-hop 연결이 실제로 어떻게 활용됐는지, (d) 최종 AI 응답을 투명하게 시각화한다(Story 1.28의 그래프 뷰 재사용/확장).
    5. AI 응답 확인 후에도 동일 세션에서 추가 질문을 계속 입력할 수 있어야 한다 — 즉 1회성 사전 예측이 아니라 실제 사용자-AI 대화가 이어지는 것처럼 여러 턴을 시뮬레이팅하며 각 턴의 판단 근거가 계속 투명하게 노출되어야 한다(Story 1.27 AC3 "부수효과 없음"은 유지하되, 세션 내 멀티턴 컨텍스트는 시뮬레이터 전용 격리 세션에서 허용).
  - **FR33-E (유형 C 하이브리드 RAG 방안, 우선순위 5, Story 1.33)**: 유형 C(포괄적 도움 요청, Story 1.15)는 특정 도메인 하나로 좁혀지지 않는 질문이라 RAG가 hybrid(여러 도메인/문서 유형에 걸친 검색 + 집계)로 동작해야 한다. 시스템은 유형 C 전용 검색 전략(다중 컬렉션/다중 도메인 동시 검색 후 카테고리별 요약 집계, `rag_strategy_hint="hybrid_multi_domain"` 신설)을 설계·구현해야 하며, 이는 FR33-D의 시뮬레이터에서 유형 C 질문 선택 시 "여러 소스가 어떻게 종합됐는지"를 다른 유형과 다른 방식(다중 hop 병렬 표시)으로 시각화해야 한다.
- **FR34 (도메인 비종속 AI 에이전트 플랫폼 전환, 2026-08-05 범위 추가, Epic 3, Story 1.34~1.40)**: 사용자가 시장·학계 리서치([MCP_VS_CLIENT_CENTRIC_UNIVERSAL_AGENT_MARKET_RESEARCH.md](../design/MCP_VS_CLIENT_CENTRIC_UNIVERSAL_AGENT_MARKET_RESEARCH.md))를 근거로, "테넌트별 데이터(API 문서+매뉴얼)만 업로드하면 어떤 REST API 기반 시스템에도 즉시 적용 가능한 말/채팅 AI 도우미"로의 전환을 요구한다. 이미 Story 1.26~1.33이 구축한 업로드·자동 구성·그래프·시뮬레이터 기반 위에서, 아래 6개 하위 요구사항을 충족해야 한다(2026-08-05 사용자 추가 요청 반영 — FR34-D/E 방향을 "시뮬레이션 유지"에서 "시뮬레이션 폐지+실제 채팅/이력 기반 투명성"으로 확정).
  - **FR34-A (실제 API 실행 지원, 우선순위 1, Story 1.35, 선행 Story 1.34 안전 설계)**: Story 1.31에서 Non-Goal로 명시든 "업로드된 설정 항목 후보의 실제 실행 매칭"을 이제 해제한다 — 업로드된 OpenAPI 스펙을 기반으로 실제 HTTP 호출을 수행하는 Tool을 자동 생성하고, LangGraph Tool-calling 경로(기존 3단계 폴백 구조 재사용)에 연결해야 한다. 실행은 리서치에서 확인한 GoEx(버클리 가리라 대학)의 "undo"/"damage confinement" 원칙을 이번 플랫폼에도 적용해야 한다 — 백엔드가 사전 정의한 메서드(GET 중심)만 기본 허용하고 POST/PUT/PATCH/DELETE는 테넌트 관리자가 명시적으로 승인한 항목만 활성화하고(화이트리스트), 가능한 경우 실행 직전 값을 기록해 되돌리기(Story 1.17 Undo Tool 패턴 재사용)를 제공해야 한다.
  - **FR34-B (Frontend IA 재편, 우선순위 2, Story 1.36)**: 현재 `settings/ai-assistant` 하위에 묘여있는 기능(매뉴얼/카탈로그/화면안내/IntelliDecision/지식베이스 현황/업로드/시뮬레이터 7개 탭)는 "설정의 하위 항목"이 아니라 독립적인 **AI 에이전트 플랫폼**이므로, 상단 네비게이션에 독립 메뉴(예: "AI 에이전트")로 승격해야 한다. 하위 구조는 기능 중심으로 재분류한다 — 예시: "지식베이스"(업로드+현황+매뉴얼 Q&A+Tool 상세를 하나로 통합), "응대 투명성"(IntelliDecision 순서도+RAG/hop 시각화+시뮬레이터 통합, FR34-D/E 참조), "설정 관리"(시스템 공통 구성, Epic 2 재사용). 재편은 기존 API 계약을 깨뜨리지 않아야 하며 UX 재구성만 한다(Story 1.30 패턴 계승).
  - **FR34-C (지식베이스 투명성 UI 고도화, 우선순위 3, Story 1.37)**: 현재 "지식베이스 현황" 탭은 집계 숫자(매뉴얼 Q&A 수/설정 항목 후보 수 등)만 보여주는데, 사용자가 요구한 "tool은 API 하나마다 정보가 자세히 보여야 하고 관련된 hop 정보도 확인"을 충족하도록, API/Tool 단위 상세 카드(엔드포인트 경로·메서드·파라미터·writable 여부·연결된 `knowledge_graph` 노드/hop 경로)를 목록→상세 드릴다운 형태로 제공해야 한다.
  - **FR34-D (IntelliDecision 설명 매뉴얼 전환, 우선순위 4, Story 1.40, 2026-08-05 수정 — 시뮬레이션 방식 폐기)**: 기존 "AI 의사결정 로직" 탭(Story 1.32)은 예시 질문을 선택하면 실제 LLM을 호출해 시뮬레이션하는 방식이었으나, 이를 **시뮬레이션 없이 지식베이스 기반으로 IntelliDecision을 설명하는 매뉴얼**로 전환한다. 유형 A~I마다 (a) 정의/트리거 예시 (b) 해당 유형이 RAG/hop/Tool과 어떻게 연계되는지에 대한 **정적 사례 설명**(업로드된 지식베이스에서 추출한 실제 예시 문서/hop 경로 기반)을 보여줘야 한다 — 실제 LLM 호출이 발생하지 않으므로 지연/비용 없이 즉시 조회 가능해야 한다.
  - **FR34-E (실제 채팅 패널 신설 — 응답 시뮬레이터 폐지, 우선순위 5, Story 1.39, 2026-08-05 확정 — "재검토"가 아니라 확정 결정)**: 기존 응답 시뮬레이터(Story 1.27/1.32)를 **삭제**하고, 대신 프론트엔드에 이미 있는 실제 채팅(SIP MESSAGE) 수신 패널(`GlobalSmsDock`, 평소에는 숨겨져 있다가 문자 수신 시만 나타나는 컴포넌트)을 **상시 노출/활용 가능하게** 확장해, 테넌트 관리자가 자기 자신(테넌트 내선)에게 문자를 보내 실제 AI 응답을 받을 수 있게 한다(`frontend/app/chat/page.tsx`의 기존 전송 로직 재사용). 이로써 "사전 시뮬레이션" 없이 **실제 대화를 통해** 기능을 검증할 수 있어야 한다.
  - **FR34-F (최근 판단 이력 고도화 — 세션 단위 유형 전환 순서도 시각화, 우선순위 6, Story 1.38,
    2026-08-05 재차 수정 — 세션 그룹핑 키를 call_id 단일 기준에서 채널별로 수정)**: 현재
    "AI 의사결정 로직" 탭 하단의 "최근 판단 이력"(Story 1.21/1.22)는 유형 코드와 근거 요약만
    표시해 정보량이 부족하다. 단순히 **개별 질의/응답 1건씩** 순서도로 보여주는 것으로는 부족하다
    — IntelliDecision은 사람과 AI가 **여러 턴을 주고받으며 유형(A~I) 사이를 옥겨 다니는 과정**
    자체로 실제 수행되기 때문이다. 따라서 이를 **동일 대화 세션 전체 단위**로 고도화해, 세션에
    속한 여러 턴이 유형 판정→RAG 검색→hop 확장→화면안내/Tool 판단→응답 생성으로 이어지는
    서브플로우가 턴마다 이어지고, 턴 사이는 "유형 전환 엣지"(유지 구간과 전환 구간을 구분)로
    연결된 하나의 flowchart로 보여줘야 한다(FR34-E의 실제 채팅 패널을 통해 발생한 멀티턴 대화
    포함). **중요한 사용자 지적(2026-08-05)**: 대화 세션을 거의 단일 call_id로만 그룹핑하려는
    것은 음성 통화에만 유효하고, SIP MESSAGE(문자)는 매 트랜잭션마다 새 call_id가 발급되므로
    동일한 기준으로 묶을 수 없다 — 채널별로 세션 그룹핑 정의를 분리해야 한다(음성 통화=call_id,
    채팅/MESSAGE=owner+caller_number+시간 윈도우). 목표는 사용자가 **프론트엔드만 확인해서**
    (실제 이력 + 지식베이스 데이터만 참조) AI가 대화를 통해 어떻게 사용자 의도를 좁혀가며 응대했는지
    이해할 수 있게 하는 것이다 — 시뮬레이션은 필요 없다(FR34-E와 연결).
  - Full MCP 서버 구현, 타 시스템의 자동 발견(discovery)은 이번에도 Non-Goal로 유지한다(업로드 기반 명시적 등록이 우선).
### Non Functional
...
- **NFR6 (판단 근거 캡처의 지연·신뢰성 예산, 2026-07-29 범위 추가)**: FR30의 판단 근거 캡처는 셀프서비스 세션의 첫 응답 지연(NFR1)을 악화시켜서는 안 되며, 캡처 메커니즘 자체의 실패(파싱 오류, 스키마 불일치 등)가 사용자에게 보여줄 응답 생성 흐름을 절대 막거나 지연시켜서는 안 된다(항상 best-effort, 실패 시 조용히 `unknown` 기록 후 계속 진행).
- **NFR7 (RAG 고도화 범위 제한, 2026-07-30 범위 추가)**: FR31-A/B는 순수 조회·메타데이터 확장이므로 기존 RAG 검색 결과·응대 품질에 회귀가 없어야 한다(NFR1 지연 예산 보호 포함). FR31-C(소스 어댑터 일반화)는 기존 매뉴얼 Q&A 색인 방식(태그 파싱)의 동작을 그대로 보존한 채 신규 어댑터를 추가하는 방식으로 구현해야 하며, 기존 색인 데이터 재색인을 강제하지 않아야 한다.
- **NFR8 (업로드 통합·자동 구성의 회귀·격리 원칙, 2026-08-04 범위 추가)**: FR33-A의 업로드 진입점 통합은 UX 계층 통합이며 `knowledge_documents`/`self_service_catalog_config` 두 저장소·API를 물리적으로 병합하지 않는다(FR32-A/Epic 2 Story 2.4~2.5의 기존 데이터·API 계약을 그대로 보존). FR33-B의 지식베이스 자동 구성은 기존 하드코딩 카탈로그/Screen Graph(Epic 2)의 조회·쓰기 동작을 대체하지 않고 병행하는 신규 지식 계층으로 구현해야 한다(get_fn/update_fn 실행부 매칭은 Non-Goal, 참고사항대로 이연). FR33-D의 시뮬레이터 멀티턴은 반드시 격리된 시뮬레이터 전용 세션에서만 컨텍스트를 유지해야 하며 실제 통화/채팅 세션 상태에는 어떤 영향도 주지 않아야 한다(Story 1.27 AC3 원칙 계승).
...
- **NFR9 (실제 API 실행 안전성, 2026-08-05 범위 추가, FR34-A)**: 업로드된 OpenAPI 스펙으로부터 자동 생성된 Tool은 **기본적으로 읽기 전용(GET)만 활성화**되며, 쓰기 메서드(POST/PUT/PATCH/DELETE)는 테넌트 관리자가 명시적으로 승인한 엔드포인트만 화이트리스트에 등록되어야 실행 가능하다(기본 거부, Epic 2의 "제외 목록 방식"과 정반대의 "화이트리스트 방식" 채택 — 임의 외부 시스템 API는 신뢰도가 검증되지 않았으므로 제외 목록 방식보다 보수적으로 접근). 실행 직전 값을 기록해 되돌릴 수 있어야 하며(undo), 실패 시 명확한 에러를 사용자에게 안내해야 한다(묵살 금지). NFR1(응답 지연) 예산은 실행 기능이 추가되어도 동일하게 유지되어야 한다.
