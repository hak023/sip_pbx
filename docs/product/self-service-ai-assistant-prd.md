...
- **FR34 (도메인 비종속 AI 에이전트 플랫폼 전환, 2026-08-05 범위 추가, Epic 3, Story 1.34~1.40)**: 사용자가 시장·학계 리서치([MCP_VS_CLIENT_CENTRIC_UNIVERSAL_AGENT_MARKET_RESEARCH.md](../design/MCP_VS_CLIENT_CENTRIC_UNIVERSAL_AGENT_MARKET_RESEARCH.md))를 근거로, "테넌트별 데이터(API 문서+매뉴얼)만 업로드하면 어떤 REST API 기반 시스템에도 즉시 적용 가능한 말/채팅 AI 도우미"로의 전환을 요구한다. 이미 Story 1.26~1.33이 구축한 업로드·자동 구성·그래프·시뮬레이터 기반 위에서, 아래 6개 하위 요구사항을 충족해야 한다(2026-08-05 사용자 추가 요청 반영 — FR34-D/E 방향을 "시뮬레이션 유지"에서 "시뮬레이션 폐지+실제 채팅/이력 기반 투명성"으로 확정).
  - **FR34-A (실제 API 실행 지원, 우선순위 1, Story 1.35, 선행 Story 1.34 안전 설계, 2026-08-06 개정 —
    임의 외부 시스템 지원을 위한 base_url/인증정보 캡처 요구사항 명시)**: Story 1.31에서 Non-Goal로 명시든
    "업로드된 설정 항목 후보의 실제 실행 매칭"을 이제 해제한다 — 업로드된 OpenAPI 스펙을 기반으로 실제
    HTTP 호출을 수행하는 Tool을 자동 생성하고, LangGraph Tool-calling 경로(기존 3단계 폴백 구조 재사용)에
    연결해야 한다. **이 시스템(SIP PBX 자체)에 국한되지 않는 임의의 외부 시스템을 대상으로 하므로**,
    업로드 시점에 다음을 반드시 캡처·저장해야 한다: (a) 대상 시스템의 전체 base URL(OpenAPI `servers`
    필드 자동 추출 또는 테넌트 직접 입력), (b) 필요 시 인증 정보(API 키/Bearer 토큰 등, 평문 로그 노출
    금지), (c) 엔드포인트별 실행 메타데이터(경로·메서드·파라미터·요청 본문 스키마)는 색인 과정에서
    유실되지 않고 실행 시점까지 조회 가능해야 한다. 실행은 리서치에서 확인한 GoEx(버클리 가리라 대학)의
    "undo"/"damage confinement" 원칙을 이번 플랫폼에도 적용해야 한다 — 백엔드가 사전 정의한 메서드(GET
    중심)만 기본 허용하고 POST/PUT/PATCH/DELETE는 테넌트 관리자가 명시적으로 승인한 항목만 활성화하고
    (화이트리스트), 실행 직전 값을 기록해 되돌리기(undo)를 제공해야 한다.
...
## Epic 4: 플랫폼 성숙화 · UX 투명성 전면 개편 (FR35)
...
- **FR35-A(업로드 완전 통합)**: `catalog`/`screen`/`kb` 탭에 나뉜 업로드 폼을 단일 드롭존 +
  파일 유형 자동 판별(확장자/MIME/내용 시그니처)로 통합. 백엔드 데이터 모델(카탈로그 JSON vs
  지식 문서)은 계속 분리 유지(NFR8 재확인).
- **FR35-B(데이터 작성 가이드)**: 테넌트가 업로드용 데이터를 직접 만들 수 있도록 지원되는 파일
  형식·최소 예시·자동 분류 규칙을 설명하는 가이드 페이지 신설.
- **FR35-C(공통 고정 콘텐츠 명시화)**: IntelliDecision 유형 A~I 레지스트리처럼 테넌트 업로드와
  무관하게 항상 존재하는 "플랫폼 공통 자산"과 "테넌트별 업로드 데이터"를 UI 라벨로 명시적으로
  구분.
- **FR35-D(응대 유형 탐색기)**: 유형 A~I 탭 + 유형별 질문 예시 선택 + RAG/Tool 필요 여부
  배지(지식베이스 기반) + 선택 시 매칭 문서·hop 경로 사전 안내(실시간 벡터 검색은 실행하되
  LLM 응답 텍스트는 생성하지 않음, Story 1.39/1.40의 "시뮬레이션 금지" 원칙 계승) + "이대로
  실제로 물어보기" 버튼으로 실제 채팅 패널(Story 1.39)에 이어지는 멀티턴 대화.
- **FR35-E(유형 C 하이브리드 시각화)**: 여러 도메인 병렬 검색 결과를 "hybrid" 같은 기술 용어
  없이 "여러 영역을 동시에 참고했다"는 문구로 번역해 탐색기 카드에 표시.
- **FR35-F(교차 탐색 + 문구 번역 계층)**: 지식베이스 항목(`qa`/`catalog`/`kb`)에서 "최근 이
  항목이 쓰인 대화 보기" 링크로 Story 1.38 세션 flowchart와 연결. hop 경로 원시 문자열과 세션
  턴의 번호 나열(① ② ③...)을 사람이 읽는 카드 UI로 번역.
- **FR35-G(MCP 서버 게이트웨이, Non-Goal 이번 범위)**: `dynamic_api_tool.py`(Story 1.35)의
  승인된 실행 Tool을 MCP 서버 Tool로 노출하는 방향 — 별도 Epic 후보로 분리, 이번 Story 목록
  미포함.
- **FR35-H(문서 전용, 레퍼런스 추적표)**: 핵심 모듈↔레퍼런스↔채택/변형/기각 사유 추적표와
  개발 배경 소개 문서 신설.
...
| Story | 제목 | FR |
|---|---|---|
| 1.41 | 업로드 진입점/폼 완전 통합 | FR35-A |
| 1.42 | 데이터 작성 가이드 문서 페이지 | FR35-B |
| 1.43 | 플랫폼 공통 vs 테넌트 데이터 배지 UI | FR35-C |
| 1.44 | 응대 유형 탐색기(IntelliDecision×실제채팅 연계) | FR35-D |
