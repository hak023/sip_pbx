...
**작성일**: 2026-07-14
**버전**: 1.5 (2026-08-05 갱신 — 도메인 비종속 AI 에이전트 플랫폼 전환 계획(FR34, Epic 3, Story 1.34~1.39) — 실제 API 실행·Frontend IA 재편·지식베이스/IntelliDecision 투명성 고도화·시뮬레이터 통합 재검토))
...
- **FR33 (업로드 통합·지식베이스 자동 구성·응답 시뮬레이터 고도화, 2026-08-04 범위 추가, Story 1.30~1.33)**: 사용자 요청(FR32 완료 후 발견된 잔여 정적 결합 + 셀프서비스 AI를 "이 시스템 전용"에서 "업로드된 상세 데이터만으로 임의 시스템에 응대 가능한 플랫폼"으로 전환)에 따라 시스템은 아래 5개 하위 요구사항을 충족해야 한다. 우선순위는 ①업로드 진입점 통합(회귀 위험 최소·즉시 체감) → ②지식베이스 자동 구성(플랫폼 전환의 핵심) → ③공통 고정 콘텐츠 화이트리스트 → ④응답 시뮬레이터/IntelliDecision UX 고도화 → ⑤유형 C 하이브리드 RAG 순서다.
  ...
  - **FR33-E (유형 C 하이브리드 RAG 방안, 우선순위 5, Story 1.33)**: 유형 C(포괄적 도움 요청, Story 1.15)는 특정 도메인 하나로 좁혀지지 않는 질문이라 RAG가 hybrid(여러 도메인/문서 유형에 걸친 검색 + 집계)로 동작해야 한다. 시스템은 유형 C 전용 검색 전략(다중 컬렉션/다중 도메인 동시 검색 후 카테고리별 요약 집계, `rag_strategy_hint="hybrid_multi_domain"` 신설)을 설계·구현해야 하며, 이는 FR33-D의 시뮬레이터에서 유형 C 질문 선택 시 "여러 소스가 어떻게 종합됐는지"를 다른 유형과 다른 방식(다중 hop 병렬 표시)으로 시각화해야 한다.- **FR34 (도메인 비종속 AI 에이전트 플랫폼 전환, 2026-08-05 범위 추가, Epic 3, Story 1.34~1.39)**: 사용자가 시장·학계 리서치([MCP_VS_CLIENT_CENTRIC_UNIVERSAL_AGENT_MARKET_RESEARCH.md](../design/MCP_VS_CLIENT_CENTRIC_UNIVERSAL_AGENT_MARKET_RESEARCH.md))를 근거로, "테넌트별 데이터(API 문서+매뉴얼)만 업로드하면 어떤 REST API 기반 시스템에도 즉시 적용 가능한 말/채팅 AI 도우미"로의 전환을 요구한다. 이미 Story 1.26~1.33이 구축한 업로드·자동 구성·그래프·시뮬레이터 기반 위에서, 아래 5개 하위 요구사항을 충족해야 한다.
  ...
  - **FR34-D (IntelliDecision 응대 투명성, 우선순위 4, Story 1.38)**: 유형별 응대 과정을 순서도/플로우차트 형태의 diagram(유형 판정→RAG 검색→hop 확장→화면안내/Tool 판단→응답 생성 단계)로 시각화해야 하며, RAG vector 검색 결과와 관련 hop이 어떻게 활용되었는지(Story 1.28 `traverse_graph`), 화면안내 hop이 어떻게 활용되었는지(Story 1.11/1.18 `rendered_by` 엣지)를 각각 단계별로 구분해 표시해야 한다.
  - **FR34-E (응답 시뮬레이터 통합/재검토, 우선순위 5, Story 1.39)**: FR34-D가 실제 대화 화면에서도 충분히 투명하게 응대 과정을 보여준다면, 기존 응답 시뮬레이터(Story 1.27/1.32)를 별도 탭으로 유지할지 혼합할지를 판단해야 한다 — 결정 기준은 "실제 대화(음성/채팅)에서도 FR34-D 투명성 패널을 그대로 노출할 수 있는가"다. 가능하다면 시뮬레이터는 별도 탭 없이 "실제 대화 + 투명성 패널"이 하나로 통합된 단일 경험으로 수렴하고, 불가능하다면(실시간 음성 파이프라인에 투명성 패널을 놓기 어려운 제약 등) 시뮬레이터를 "사전 검증용 도구"로 명시적으로 재정의해 유지한다.
