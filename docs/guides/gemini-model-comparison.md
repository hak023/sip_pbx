# Gemini 2.5 Flash vs 1.5 Pro 모델 비교

## 📊 모델 개요

Google Gemini는 다음 모델을 제공합니다:
- **Gemini 2.5 Flash**: 최신 Flash 모델, 빠른 응답, 비용 효율성 최적화 ⭐ **현재 사용 중**
- **Gemini 1.5 Pro**: 고성능, 복잡한 작업에 최적화

> **현재 시스템**: `gemini-2.5-flash` 사용 중 (config.yaml)

---

## ⚡ Gemini 2.5 Flash 특징 (현재 사용)

### 핵심 특징

1. **최신 모델 (2024년 12월 출시)**
   - Gemini 1.5 Flash의 개선 버전
   - Pro보다 **2~3배 빠른 응답 속도**
...
ai_voicebot:
  ...
  google_cloud:
    ...
    gemini:
      model: "gemini-2.5-flash"        # 최신 Flash 모델로 변경
...
async def choose_model(user_question: str) -> str:
    ...
    if any(p in user_question for p in simple_patterns):
        ...
        return "gemini-2.5-flash"  # 대부분 Flash로 충분
```
