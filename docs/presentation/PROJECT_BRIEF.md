...
기획 초기에 세운 세 가지 질문은, 실제 시스템에서는 **지식 축적·검색**, **의도 기반 행동 선택**, **체감 품질·신뢰(HITL)** 로 그대로 이어집니다. 아래에서는 질문에 대한 방향과, 그것이 데이터·파이프라인으로 어떻게 구현되었는지를 **한 흐름**으로 정리합니다.

---

### 1. "구축하지 않고도 AI가 알아서 지식을 만들 수 없을까?"

**방향**  
사람이 미리 FAQ를 채우는 **구축형** 부담을 줄이고, **상담원↔고객 통화**를 그대로 지식으로 쓴다. 전화로 오간 답변이 다음 통화의 근거가 된다.

**구현으로 녹인 모습**  
통화가 발생하면 **실시간 STT**로 텍스트화하고, **화자 분리(Diarization)** 로 고객 발화는 질문·상담원 발화는 답변 쪽으로 태깅합니다. 의미 단위 **청킹** 뒤 LLM이 인사말·잡담을 걸러 **Q&A 쌍**을 추출하고, 문제 유형·해결 방법·카테고리·고객 감정 등 **메타데이터**를 붙입니다. 코사인 유사도(예: > 0.9)로 **중복을 검사**한 뒤 **ChromaDB(Vector DB)** 에 **테넌트(내선)별**로 적재합니다.

쌓인 조각은 저장만 되는 것이 아니라, 응답 시점에 **임베딩 검색**으로 꺼내 **프롬프트 컨텍스트**를 만들고 LLM에 주입합니다. 단순 키워드 매칭이 아닌 **Active RAG**이며, 지식베이스가 갱신되면 검색 결과가 즉시 바뀝니다. 필요 시 **API·MCP** 등 외부 정형 데이터와 합쳐 근거를 보강할 수 있습니다.

**이렇게 얻는 것**  
별도 대규모 구축 없이 통화가 쌓일수록 지식이 자라고, 벡터 검색·테넌트 격리로 **환각을 줄이면서** 최신 내용이 곧바로 응답에 반영됩니다.

---

### 2. "AI로 ARS를 완전히 대체할 수 있을까?"

**방향**  
고정 트리와 "1번 누르세요" 대신, **목적(Goal)** 과 **역할(페르소나)·업무 범위**만 주면 AI가 맥락에 맞는 다음 행동을 고르게 한다.

**구현으로 녹인 모습**  
고객 **자연어 발화**마다 LLM이 **의도(Intent)를 추론**합니다(시스템에서는 **17가지 의도 분류**). **LangGraph** 기반 대화 그래프가 그다음 경로를 선택합니다 — API 호출, **호 전환**, 안내 멘트, **지식 검색** 등 목적 달성에 필요한 조합을 쓰며, ARS 단계를 기계적으로 밟지 않아도 됩니다. "반품하고 교환하고 싶어요"처럼 **복합 의도**도 한 흐름에서 처리할 수 있습니다.

**이렇게 얻는 것**  
시나리오 코드 전면보다 **페르소나·범위 설정** 중심으로 운영이 단순해지고, 고객은 말한 그대로를 시스템이 이해하는 경험을 갖습니다.

---

### 3. "AI 응대의 만족도를 사람 수준으로 끌어올릴 수 없을까?"

**방향**  
불만의 뼈대는 세 가지였습니다. **①** 답변 풀이 좁다. **②** 기계 같고 느리다. **③** 틀려도 말이 나간다. 이에 **추론+RAG**로 범위와 근거를 넓히고, **스트리밍**으로 첫 소리까지의 시간을 줄이며, **신뢰도**가 낮을 때 **운영자(HITL)** 가 끼어들고, 그 결과가 다시 지식이 되게 한다.

**구현으로 녹인 모습**  
- **범위·정확도**: 1번에서 쌓인 인덱스를 **RAG**로 붙여 추론과 결합합니다. 응답에 **신뢰도(Confidence)** 를 두고, 임계 미만이면 **HITL Shadowing**으로 넘깁니다. 운영자에게 실시간 알림·질문이 전달되고, 입력한 답은 정제되어 **TTS**로 송출되며 **동시에 지식베이스에 저장**되어 다음에는 AI가 단독으로 답할 수 있습니다.  
- **자연스러움·지연**: **Streaming Architecture** — LLM이 **첫 토큰**을 내는 즉시 **TTS**가 합성·송출을 시작하고, STT→처리→TTS 파이프라인은 **비동기**로 맞물려 체감 지연을 줄입니다.  
- **통화 후**: 필요 시 문자·Happy Call 등 **후속 조치**로 이어갈 여지를 둡니다.

**이렇게 얻는 것**  
짧은 반응 시간, 운영자가 AI를 **실시간으로 보조**하는 Shadowing, 보정 한 번이 **VectorDB·지식층**을 두껍게 하는 선순환입니다.

---

세 축은 **한 통화 안에서 동시에** 돌아갑니다. 같은 대화 속에서 지식이 쌓이고(RAG의 재료), 의도에 따라 행동이 갈라지며, 들을 만한 속도와 사람의 안전장치(HITL)가 겹칩니다.
