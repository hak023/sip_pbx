...
- 타이머 기반: `no_answer_timeout` 경과 후 AI 활성화
- 수동 설정: 웹에서 "부재중" 설정 시 즉시 AI 활성화
- 실시간 STT/TTS/LLM 파이프라인 작동
...
### 테스트 케이스 2: AI 응대 모드 타이머 기반 활성화

**목적**: 타이머 기반 AI 응대 모드 활성화 시간 검증
...
@pytest.mark.asyncio
async def test_ai_attendant_timer_activation():
    """타이머 기반 AI 응대 모드 활성화 시간 테스트"""
    call_manager = CallManager(...)
    
    start = time.time()
    
    # 전화 수신 시뮬레이션
    await call_manager.handle_incoming_call(call_id="test-001", callee="1004")
    
    # 타이머 대기 (10초)
    await asyncio.sleep(10.1)  # 약간 여유
    
    # AI 활성화 확인
    assert call_manager.is_ai_mode_active("test-001")
    
    activation_time = (time.time() - start) * 1000
    
    # 목표: 10.5초 이내 (타이머 + 여유)
    assert activation_time < 10500, f"활성화 시간 초과: {activation_time}ms"
```

---

### 테스트 케이스 3: AI 응대 모드 수동 설정 활성화

**목적**: 수동 부재중 설정 시 즉시 AI 응답 검증
...
```python
@pytest.mark.asyncio
async def test_ai_attendant_manual_activation():
    """수동 부재중 설정 시 AI 응대 모드 활성화 시간 테스트"""
    from src.sip_core.operator_status import get_operator_status_manager
    
    status_manager = get_operator_status_manager()
    status_manager.set_away("1004", "회의 중입니다")
    ...
    # 전화 수신 시뮬레이션
    await call_manager.handle_incoming_call(call_id="test-002", callee="1004")
    
    # 즉시 AI 활성화 확인
    assert call_manager.is_ai_mode_active("test-002")
    
    activation_time = (time.time() - start) * 1000
    
    # 목표: 0.2초 이내 (즉시 활성화)
    assert activation_time < 200, f"활성화 시간 초과: {activation_time}ms"
```

---

### 테스트 케이스 4: AI 응대 모드 STT/TTS 지연시간

**목적**: AI 응대 모드에서 STT/TTS 지연시간 검증

**Given**:
- AI 응대 모드 활성화
- 실시간 STT/TTS 파이프라인 작동
...
**Then**:
- STT 최종 결과: 150~400ms 이내
- TTS 첫 청크: 200~400ms 이내
- 전체 응답 시간: 1.5초 이내

**검증 방법**:
```python
@pytest.mark.asyncio
async def test_ai_attendant_stt_tts_latency():
    """AI 응대 모드 STT/TTS 지연시간 테스트"""
    orchestrator = AIOrchestrator(...)
    
    # STT 시간 측정
    stt_start = time.time()
    user_text = await orchestrator.wait_for_user_speech()
    stt_time = (time.time() - stt_start) * 1000
    
    # TTS 시간 측정
    tts_start = time.time()
    await orchestrator._speak("안녕하세요")
    tts_first_chunk = (time.time() - tts_start) * 1000
    
    # 목표 검증
    assert stt_time < 400, f"STT 시간 초과: {stt_time}ms"
    assert tts_first_chunk < 400, f"TTS 첫 청크 시간 초과: {tts_first_chunk}ms"
```
...
**총 응답 시간**: **11.3~11.4초** (타이머 + AI 응답)

---

#### 수동 부재중 설정 활성화
**설정**: 웹 API `/api/operator/status` → `"status": "away"`
...
**총 응답 시간**: **1.35~1.41초** (즉시 응답)

---

### AI 응대 모드 vs 일반 통화 응답 시간 비교

| 모드 | 활성화 시간 | 첫 응답 시간 | 총 시간 |
|------|------------|------------|---------|
| **타이머 기반** | 10초 | 1.3초 | **11.3초** |
| **수동 설정** | 0.06초 | 1.3초 | **1.36초** |
| **일반 통화** | - | - | 착신자 응답 시간에 의존 |

**분석**:
- ✅ **수동 설정**: 거의 즉시 AI 응답 (1.36초)
- ⚠️ **타이머 기반**: 10초 대기 후 응답 (11.3초)
- 💡 **권장**: 중요한 통화는 수동 설정 사용

---

### AI 응대 모드 STT/TTS 지연시간 분석

#### STT (Speech-to-Text) 지연시간
**AI 응대 모드에서의 STT**:
- **실시간 스트리밍**: 통화 중 계속 실행
- **VAD 감지 후**: 100~300ms 내 최종 결과
- **화자 분리**: Diarization 포함 시 +50~100ms

**예상 시간**: **150~400ms** (VAD 감지 후)

#### TTS (Text-to-Speech) 지연시간
**AI 응대 모드에서의 TTS**:
- **첫 청크 생성**: 200~400ms
- **스트리밍 시작**: 첫 청크 생성 즉시
- **전체 생성**: 응답 길이에 비례
...
| **최악 조건** | 2.4초 | ⚠️ 수용 가능 |
| **최적화 후** | 0.8초 | ✨ 거의 즉각 반응 |
| **AI 응대 (타이머)** | 11.3초 | ⚠️ 대기 시간 포함 |
| **AI 응대 (수동)** | 1.36초 | ✅ 즉시 응답 |
...
4. **AI 응대 모드 최적화**:
   - **타이머 기반**: `no_answer_timeout` 조정 (5~15초 권장)
   - **수동 설정**: 중요 통화는 즉시 AI 응답 (1.36초)
   - **STT 최적화**: 실시간 스트리밍 지연 최소화
   - **TTS 최적화**: 첫 청크 생성 시간 단축
...
1. **타이머 기반 대기 시간**
   - **문제**: `no_answer_timeout` (10초) 동안 발신자가 대기해야 함
   - **영향**: 사용자 경험 저하 가능성
   - **해결책**: 수동 부재중 설정 사용 권장

2. **STT Diarization 지연**
   - **문제**: 화자 분리 시 추가 50~100ms 지연
   - **영향**: 첫 응답 시간 약간 증가
   - **해결책**: 비동기 처리 또는 선택적 활성화

3. **네트워크 지연**
   - **문제**: Google Cloud API 호출 시 네트워크 지연
   - **영향**: STT/TTS/LLM 응답 시간 변동
   - **해결책**: 리전 최적화 또는 캐싱
...
**AI 응답 시간 메트릭**:
- `ai_response_time_seconds`: 전체 응답 시간 (Histogram)
- `rag_search_time_seconds`: RAG 검색 시간 (Histogram)
- `llm_generation_time_seconds`: LLM 생성 시간 (Histogram)
- `tts_first_chunk_time_seconds`: TTS 첫 청크 시간 (Histogram)

**AI 응대 모드 메트릭**:
- `ai_attendant_activation_time_seconds`: 활성화 시간 (Histogram)
- `ai_attendant_timer_based_count`: 타이머 기반 활성화 횟수 (Counter)
- `ai_attendant_manual_count`: 수동 설정 활성화 횟수 (Counter)

**STT/TTS 메트릭**:
- `stt_final_result_time_seconds`: STT 최종 결과 시간 (Histogram)
- `tts_first_chunk_time_seconds`: TTS 첫 청크 시간 (Histogram)
