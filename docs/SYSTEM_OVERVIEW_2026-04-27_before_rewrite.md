...
**베이스라인**: 본문의 제품·아키텍처 서술은 2026-04-08 시점을 기준으로 하며, **2026-04-08 이후** 반영 사항은 **§1.4**에 요약한다.
...
| **지능형 AI 응대** | 부재중/영업시간 외 자동 응답, RAG 기반 지식 검색, 18가지 의도 분류 |
...
### 1.4 2026-04-08 이후 개발 요약 (reports 기준)

`docs/reports/2026-04/` 일자별 리포트(특히 2026-04-09~2026-04-23)와, 필요 시 **다른 월 폴더**의 연관 항목을 함께 본다. 아래는 그중 시스템 개요에 영향이 큰 변경만 압축한 것이다. 구현·파일·API 세부는 각 리포트를 우선한다.

| 영역 | 요약 | 대표 리포트 (예) |
|------|------|------------------|
| **착신 제어 (Call Control)** | `config.yaml`·헤더 토글에 흩어지던 정책을 **라우팅 규칙 + 스케줄 + 안내멘트**로 통합. SQLite `data/call_control.db`, REST `/api/call-control/*`, 프론트 `/settings/call-control`. 모드: 직접 연결, N초 후 AI, **즉시 AI**, 전달, 링 그룹(모델·DB 수준) 등. SIP `INVITE` 경로에서 발신자 필터→스케줄→기존 `operator_status` 폴백. 인사 TTS는 `greeting_override`로 주입. | `2026-04-14_1700_CALL_CONTROL_IMPL.md` |
| **HITL 에스컬레이션** | AI 한계 시 동작을 페르소나에서 선택: **기존 HITL** 또는 **상담원 직접 연결(내선·호전환)**. | `2026-04-09_1300_ESCALATION_MODE_DESIGN.md` |
| **통화 연결음 (Ringback)** | Early media 구간에 **TTS 인사 + Suno 연결음** RTP 스트리밍, `/api/ringback/*`, `/settings/ringback`, 200 OK·AI 인수 시 정지. ngrok·콜백·가사/스타일·KB 폴백 등 운용 리포트 다수. | `2026-04-13_2230_RINGBACK_FEATURE_IMPL.md` 등 |
| **통화이력·미해결** | 통화 단위 `is_unresolved`, AI 미처리 시 자동 플래그, `PATCH` 해제, `batch-init-unresolved`, 대시보드 **미해결 통화** 지표, SMS UI를 **미처리** 흐름에 통합. | `2026-04-13_2355_UNRESOLVED_CALL_MANAGEMENT.md` |
| **글로벌 콜 독·CID** | 대시보드 밖에서도 `call_started` 등으로 **플로팅 콜 독**·알림, `GET /api/call-history/caller-context`로 직전 인입 맥락, `?call_id=`로 대시보드 피드 연동. 이후 **연락처 트리·드래그앤드롭·듀얼 라인** 등 CID/Contacts 확장. | `2026-04-16_0930_GLOBAL_CALL_DOCK_AND_CALLER_CONTEXT_API.md` 등 |
| **SIP 메시징·채팅** | `SIP MESSAGE`·채팅 릴레이(내선/브랜치), UTF-8·CPIM, 설정 UI 이전, RCS/문자·통화 연계 리포트. | `2026-04-14_1538_CHAT_MANAGEMENT_IMPLEMENTATION_AUDIT.md` 등 |
| **예약·캘린더·알림** | **Google Calendar** OAuth(오너 DB), **음성 예약 자동화**(`booking_intent_heuristic`, CDR `booking_*` 이벤트, 스키마 버전 상향), Gemini **Native FC**·구조화 STT, 예약 **SMS/RCS** 흐름, `booking` DB·폴더 마이그레이션. | `2026-04-20_1538_VOICE_BOOKING_AUTO_IMPL.md`, `2026-04-21_0945_BOOKING_SMS_RCS_IMPL.md` 등 |
| **Voicebot·툴** | Booking/RAG/FAQ/URL fetch 등 **도구·도메인 설정** 확장, 통화이력·전달 대상(Forward)·연락처 KB와 **호전환** 연동 검토. | `2026-04-09_1400_VOICEBOT_TOOL_EXPANSION_BEYOND_BOOKING.md` 등 |
| **LangGraph·런타임** | `langgraph-checkpoint-sqlite`를 AI 의존성에 명시, Sqlite checkpointer 기본 사용 유도, async SQLite checkpointer·graceful shutdown 관련 보완. | `2026-04-15_1635_LANGGRAPH_CHECKPOINT_SQLITE_DEPS.md` |
| **인프라·품질** | gRPC/이벤트 루프(Windows), embedded Uvicorn 시그널·종료, **액세스 토큰** `tok_` 프리픽스·WS, LLM `max_output_tokens` 초과 시 **TTS용 텍스트 정리**, `start-all`·ngrok 연동. | `2026-04-14_1610_MAX_TOKENS_TTS_SANITIZE_IMPL.md` 등 |
...
`out_of_scope`, `nlu_fallback`, `booking`

**18개 의도 분류 및 처리 경로 상세**
...
| **E: 예약 처리** | booking | 키워드 조기 분류(~2ms) 또는 LLM + Tool Use loop → booking_agent_node | 0회 또는 1+N회 | SQLite booking DB |
| **F: 특수 처리** | transfer | 호 전환 로직 실행 (TransferManager) | 0회 | 없음 |
| **G: 범위 외** | out_of_scope | 페르소나 기반 범위 외 응답 | 0~1회 | persona KB |
| **H: 폴백** | nlu_fallback | HITL 에스컬레이션 또는 모름 응답 | 1회 | 없음 |
...
**에스컬레이션 모드 (2026-04 이후)**: 페르소나 설정에 따라 AI 한계 시 **HITL 대시보드 요청**과 **지정 내선으로의 호전환**을 선택할 수 있다. 상담원 직접 연결 모드는 운영자 타이핑 없이 TTS 안내 후 **기존 TransferManager 등 호전환 경로**로 이어질 수 있다(설계: `2026-04-09_1300_ESCALATION_MODE_DESIGN.md`).
...
| 화면 | 기능 |
|---|---|
| **대시보드** | 메트릭 카드(미해결 통화 건수 등), 실시간 통화 카드, STT/TTS 피드, 처리 로그, HITL 응답, 통화이력, `?call_id=`로 특정 통화 피드 정렬(전역 콜 독과 연동) |
| **지식·설정** | 지식 CRUD, TXT 업로드, **페르소나(에스컬레이션 모드)**, **착신 제어(Call Control)**, **통화 연결음(Ringback)** 등 설정 화면 |
| **통화이력** | 전체 통화 목록, 녹음 재생, 트랜스크립트, CDR 상세, **미해결/해결** 토글·SMS(미처리 흐름) |
| **로그인** | 테넌트(내선번호) 선택 로그인 |
| **전역 UI** | 로그인 후 **글로벌 콜 독**(다른 페이지에서도 인입 알림·STT/TTS), 연락처·Call Dock 확장(리포트 기준 단계적 도입) |
...
**기능**: LLM Tool Use 기반 음성 예약 처리 + REST API + 웹 관리 UI

**유저 스토리**: 고객이 "내일 오후 3시에 2명 예약해줘"라고 말하면, AI가 가용 슬롯을 확인하고 예약을 생성한 뒤 확인 안내를 즉시 음성으로 전달한다.

**동작 흐름**:
```
고객 발화 ("예약", "예약 취소" 등 키워드)
  → [0차 키워드 분류 → booking intent, ~2ms]
    → booking_agent_node 진입
      → LLM + bind_tools(BOOKING_TOOLS)
          ┌──────────────────────────────────────────┐
          │  Tool 호출 루프 (최대 _MAX_TOOL_ROUNDS)   │
          │  LLM이 필요한 Tool을 자율 선택·호출:      │
          │  check_available_slots → create_booking  │
          │  또는 get_booking_info → cancel_booking  │
          └──────────────────────────────────────────┘
      → 최종 텍스트 응답 → TTS 송출
```

**핵심 설계**:
- **도메인 독립**: `booking_settings` 테이블의 `domain_type` + `booking_schema_fields`로 레스토랑·병원·미용실 등 모든 업종에 적용 가능
- **낙관적 잠금**: `BEGIN IMMEDIATE` 트랜잭션으로 동시 예약 시 슬롯 초과 방지
- **Tool 직접 호출**: LLM이 HTTP가 아닌 Python 함수를 직접 호출 (오버헤드 없음)
- **Progressive Slot Filling**: 정보 미입력 시 LLM이 자연스럽게 추가 질문하며 수집

**DB 스키마 (SQLite `data/booking.db`)**:

| 테이블 | 설명 |
|---|---|
| `booking_settings` | 테넌트별 도메인 설정 (서비스명, 슬롯 길이, 확인 메시지 템플릿 등) |
| `booking_slots` | 예약 가능 슬롯 (날짜·시간·용량·차단 여부) |
| `bookings` | 예약 내역 (고객 정보·슬롯·상태·추가 데이터 JSON) |
| `booking_schema_fields` | 도메인 맞춤 추가 수집 필드 정의 |

**관련 파일**:
- `src/booking/database.py` — SQLite 연결·초기화·DDL
- `src/booking/models.py` — Pydantic 모델
- `src/services/booking_service.py` — 비즈니스 로직
- `src/api/routers/booking.py` — REST API 라우터
- `src/ai_voicebot/langgraph/tools/booking_tools.py` — LangChain Tool 정의
- `src/ai_voicebot/langgraph/nodes/booking_agent.py` — LangGraph 노드
- `frontend/app/booking/` — Next.js 관리 UI (3개 페이지)

### 2.13 멀티 테넌트 구조 (테넌트별 독립 AI)
...
| **메트릭** | GET | `/api/metrics/dashboard` | 대시보드 메트릭(미해결 통화 건수 등) |
| **통화·CID** | GET | `/api/call-history/caller-context` | 직전 인입 발신자 맥락(CID) |
| **통화이력** | PATCH | `/api/call-history/{id}/resolve` | 미해결/해결 토글 |
| **통화이력** | POST | `/api/call-history/batch-init-unresolved` | `is_unresolved` 일괄 보정(선택) |
| **착신 제어** | * | `/api/call-control/*` | 규칙·스케줄·안내멘트·필터 등(상세는 라우터) |
| **통화 연결음** | * | `/api/ringback/*` | Suno·설정·미리듣기 등(상세는 라우터) |
...
| **예약·슬롯** | GET/POST | `/api/booking/slots` | 슬롯 목록 조회 / 생성 |
| **예약·슬롯** | PUT/DELETE | `/api/booking/slots/{id}` | 슬롯 수정 / 삭제 |
| **예약** | GET/POST | `/api/booking` | 예약 목록 조회 / 생성 |
| **예약** | GET/PUT/DELETE | `/api/booking/{id}` | 예약 상세·수정·취소 |
| **예약 설정** | GET/PUT | `/api/booking/settings/{owner}` | 도메인 설정 조회·저장 |
| **예약 필드** | GET/POST | `/api/booking/fields/{owner}` | 커스텀 필드 목록·생성 |
| **예약 필드** | PUT/DELETE | `/api/booking/fields/{owner}/{id}` | 커스텀 필드 수정·삭제 |
...
| 대화 그래프 | LangGraph (StateGraph) + Tool Use (LangChain) |
| WebSocket | Socket.IO (python-socketio, aiohttp) |
| 벡터 DB | ChromaDB |
| 관계형 DB | SQLite (`booking.db` 예약, `call_control.db` 착신 제어·스케줄, 통화/연락처 등 기타) |
...
| 복잡한 질문 | ~3~5초 | rewrite_query·RAG 2-pass 등 (step_back 경로는 2026-04-03 제거) |
| 예약 처리 (키워드) | ~0.002초 | 키워드 조기 분류 후 booking_agent 직접 라우팅 |
| 예약 처리 (LLM+tool) | ~2~5초 | LLM + Tool 호출 루프 (슬롯 확인·예약 생성) |
...
| [reports/2026-04/](reports/2026-04/) | 2026년 4월 분석·점검 리포트(2026-04-08 이후의 착신 제어, 링백, 미해결 통화, 콜 독, SIP 메시징, 예약·SMS·캘린더 등 **대부분이 일자별로 축적**됨) |

### 예약 시스템

| 문서 | 설명 |
|---|---|
| [reports/2026-04/2026-04-08_1450_AI_VOICEBOT_BOOKING_SYSTEM_DESIGN.md](reports/2026-04/2026-04-08_1450_AI_VOICEBOT_BOOKING_SYSTEM_DESIGN.md) | AI 예약 시스템 상세 설계 문서 |
