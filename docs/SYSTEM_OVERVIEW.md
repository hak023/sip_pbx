...
### 1.4 셀프서비스 AI 도우미의 개발 배경 (왜 이렇게 발전했는가)

셀프서비스 AI 도우미(테넌트 관리자가 통화·문자·웹채팅으로 시스템 사용법·설정·통계를 AI에게
물어보는 기능, §4.11)는 아래 3단계를 거쳐 지금의 형태가 되었다. 각 단계에서 참고한 연구·시장
사례는 [레퍼런스 추적표](design/REFERENCE_TRACEABILITY.md)에 모듈 단위로 정리되어 있다.

| 단계 | 동기 | 산출물 |
|---|---|---|
| **1. 문제 인식** | 고객센터에 서비스 이용법 문의·UI 개편 후 혼란이 집중 — 전화/문자/웹채팅에서 AI가 설정 변경·통계 조회까지 대신 처리하면 고객 경험이 개선될 것이라는 가설 | Epic 1 — 셀프서비스 Tool-calling, 설정 카탈로그, Screen Graph, IntelliDecision 유형 A~I |
| **2. 범용화 전환** | "이 도메인에만 묶이지 말고, REST-API가 있는 모든 서비스가 업로드만으로 AI 도우미를 쓸 수 있게 하자"는 재정의. 시장조사 결과 client-centric 접근의 선행 연구/상용 사례(RestGPT, GPT Actions, mcp-link 등)가 이미 존재함을 확인 | Epic 2(카탈로그/Screen Graph 동적화), 도메인 비종속 지식베이스 업로드(문서 CRUD, 자동 구성) |
| **3. 투명성 전환** | 지식베이스가 커질수록 "AI가 왜 이렇게 답했는가"가 블랙박스화되는 문제를 인식 — 신뢰 확보와 "데이터만 바꾸면 AI 응대가 바뀐다"는 사용자 통제감을 동시에 원함 | IntelliDecision 정책 레지스트리, hop 시각화, 실제 채팅 패널, 세션 단위 응대 순서도, 응대 유형 탐색기 |

상세 배경 서술과 착수 시점의 갭 진단은 [Epic 4 계획 리포트](reports/2026-08/2026-08-06_epic4_platform_maturation_and_ux_transparency_planning.md) §1~2를 참고.
...
- **IntelliDecision 유형 C(포괄적 도움 요청, Story 1.15)**: "AI가 뭘 할 수 있어?"처럼 특정 기능을 지정하지 않은 포괄적 질문에 대해, 기존 유형 A(탐색성)/유형 B(실행성)와 구분되는 별도 유형으로 실제 구현된 능력(설정 조회·자동설정·통계·통화 이력 NLQ·온보딩·매뉴얼 Q&A)을 구체적 예시와 함께 안내한다. Tool 호출이 필요 없어 기본 시스템 프롬프트에 포함되며, bind_tools/Gemini FC/프롬프트 폴백 3개 경로 모두에서 항상 동작한다.
