...
| [SELFSERVICE_AI_ASSISTANT_INTRODUCTION.md](SELFSERVICE_AI_ASSISTANT_INTRODUCTION.md)                                                                             | **셀프서비스 AI 도우미 서비스 소개서** — 배경·장점·타겟유저·핵심기능(다이어그램·시장조사 포함)·유저스토리 5개 상세 (2026-08-10 신규) |
...
| [self-service-ai-assistant-e2e-manual-driven-qa-plan.md](qa/self-service-ai-assistant-e2e-manual-driven-qa-plan.md) | **(2026-08-07, v1.0, 실행 대기)** 1001 테넌트 × 가상 의류판매 관리자 시뮬레이터로 "낯선 원격 시스템 + 자연어 매뉴얼 업로드"만으로 안내·조회·설정변경이 가능한지 검증하는 정식 QA 계획. IntelliDecision 유형 A~I + N홉 27개 케이스, 로직 다이어그램(라우팅/Hop/상태제어) 포함. 픽스처: `fixtures/clothing-store-assistant-manual.md`, `scripts/qa_clothing_store_admin_simulator.py`. §5에 기록된 갭은 Story 1.51로 구현 완료(실서버 IV는 이 QA 실행이 겸함)                                                                                     |
