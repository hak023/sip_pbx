...
| [AI_SERVICE_AGENT_SERVICE_INTRODUCTION.md](AI_SERVICE_AGENT_SERVICE_INTRODUCTION.md)                                                                                   | **지능망 AI Service Agent PoC 소개서** — 10초 무응답 착신전환 단일 시나리오를 기준으로 IntelliDecision·N-hop RAG·Dynamic Tool Wrapper의 작동 원리, Zendesk/Babbel 대표 사례, Builder 중심 AICC 비교를 설명 (v6.4) |
| [SELFSERVICE_AI_ASSISTANT_INTRODUCTION.md](SELFSERVICE_AI_ASSISTANT_INTRODUCTION.md)                                                                                   | **Self-Service AI Assistant 참고·백업 자료집** — 임원형 초안과 상세 설명의 보관본 (v5.0)                                                                                                                          |
| [reports/2026-09/2026-09-03_ai-service-agent-introduction-enhancement-plan.md](reports/2026-09/2026-09-03_ai-service-agent-introduction-enhancement-plan.md)           | **소개서 보강 계획** — 핵심 기술 상세화, Builder 중심 AICC 비교, 대표 시장 사례 압축, 문체·문서 정합성 교정 방향 (검토 요청)                                                                                      |
| [QUICK_START.md](QUICK_START.md)                                                                                                                                       | 설치 및 실행 가이드                                                                                                                                                                                               |
| [reports/2026-09/2026-09-02_ai-service-agent-core-capability-restructure-review.md](reports/2026-09/2026-09-02_ai-service-agent-core-capability-restructure-review.md) | AI Service Agent 소개서 4번 장의 IntelliDecision·N-hop RAG·Dynamic Tool Wrapper 중심 재구성 검토                                                                                                                  |
...
추가 점검 리포트: [2026-08-06_frontend_ia_and_kb_ux_review.md](reports/2026-08/2026-08-06_frontend_ia_and_kb_ux_review.md) — ①"AI 에이전트" 메뉴 중복(MAIN_NAV+SETTINGS_NAV) ②지식베이스 6개 탭 파편화 개선 방향 ③raw JSON 노출 3곳 확인 + 업로드 테스트 데이터(`docs/test_data/`) 제공
...
| 1.43  | [1.43.platform-vs-tenant-data-origin-badge.story.md](stories/1.43.platform-vs-tenant-data-origin-badge.story.md)                                     | Review(플랫폼 공통 vs 테넌트 데이터 배지 UI, FR35-C — OriginBadge 구현, 실서버 IV 잔여)                                                                                                        |
...
| 1.47  | [1.47.reference-traceability-and-development-background.story.md](stories/1.47.reference-traceability-and-development-background.story.md)           | Done(문서 전용, 레퍼런스 추적표 + 개발 배경 소개, FR35-H — REFERENCE_TRACEABILITY.md 신규 작성 완료)                                                                                           |
| 1.48  | [1.48.unified-knowledge-list-view.story.md](stories/1.48.unified-knowledge-list-view.story.md)                                                       | Superseded(클릭 기반 리스트 뷰, 2026-08-06 사용자 피드백으로 Story 1.49의 `KnowledgeClusterTable`로 대체됨 — qa 그룹핑 로직은 계승)                                                            |
| 1.49  | [1.49.unified-knowledge-detail-panel.story.md](stories/1.49.unified-knowledge-detail-panel.story.md)                                                 | Review(지식 그룹(hop 연결 성분) 테이블 뷰, FR36-B — 2026-08-06 사용자 피드백으로 클릭 기반 리스트+상세를 폐기하고 `KnowledgeClusterTable`로 재설계)                                            |
| 1.50  | [1.50.upload-success-auto-focus.story.md](stories/1.50.upload-success-auto-focus.story.md)                                                           | Review(업로드 완료 → 신규 항목 자동 포커스, FR36-C — kb 탭 자동이동+hop 자동펼침+강조표시 구현, 실서버 IV 잔여)                                                                                |
| 1.51  | [1.51.dynamic-api-tool-agent-integration.story.md](stories/1.51.dynamic-api-tool-agent-integration.story.md)                                         | Review(QA 계획 작성 중 발견한 갭 보완 — 업로드된 OpenAPI 문서의 승인된 메서드를 LangGraph Tool-calling 루프에 실제 연결, 유형 B/E 실행 지원. 신규 단위테스트 6건, 실서버 IV는 QA 실행 시 겸함) |

계획 리포트: [2026-08-06_selfservice_kb_unified_view_plan.md](reports/2026-08/2026-08-06_selfservice_kb_unified_view_plan.md) — 6개 탭 파편화·raw 지표 진단 + 통합 리스트/상세 패널 UI 미리보기(FR36 근거)
...
### Epic 8 — MCP 서버 게이트웨이 (동적 REST-API Tool 노출) (2026-08-10 신설)

> 업로드된 OpenAPI 스펙 기반의 동적 REST-API Tool을 MCP 프로토콜로 외부 클라이언트(Claude Desktop, VS Code Copilot 등)에 노출한다.  
> `build_dynamic_tools_for_owner()`(Story 1.51)를 FastMCP로 감싸는 얇은 어댑터 — 새 비즈니스 로직 없음.  
> 계획 리포트: [2026-08-10_epic5_mcp_server_gateway_planning.md](reports/2026-08/2026-08-10_epic5_mcp_server_gateway_planning.md)

| Story | 문서                                                                                                     | 상태                                                                             |
| ----- | -------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| 8.1   | [8.1.mcp-gateway-dynamic-api-get-tools.story.md](stories/8.1.mcp-gateway-dynamic-api-get-tools.story.md) | Done(FastMCP 기반 + GET/PUT/Undo Tool 노출, 단위테스트 11건 PASS, IV 완료)       |
| 8.2   | [8.2.mcp-gateway-write-undo-tools.story.md](stories/8.2.mcp-gateway-write-undo-tools.story.md)           | Draft(Story 8.1에서 쓰기/Undo Tool이 이미 포함됨, Claude Desktop 연결 IV만 잔여) |
...
| [self-service-ai-assistant-tenant-simulation-qa-plan.md](qa/self-service-ai-assistant-tenant-simulation-qa-plan.md)              | **(2026-08-28, v2.1, 실행 완료 — 신규 셀프서비스 E2E QA는 이 문서를 사용)** 테넌트 시뮬레이션 E2E QA 계획·실행 기록서. 1002(브루앤빈 카페 오더, 주력) + 1001(런웨이 클로젯 의류, 대조군) **총 52건 중 50건 실행 → PASS 34 / FAIL 16 / N/A 2**. 케이스마다 목적·전제·입력 발화 원문·기대 흐름도·정상 판정 기준·**케이스별 판정 근거**(§7.2/7.3)를 수록. 테넌트 격리·미승인 쓰기 차단·N-hop 순회 PASS, Undo·자연어 ID 매핑 FAIL. 픽스처: `fixtures/{cafe-order,clothing-store}-assistant-manual.md`, `scripts/qa_{cafe_order,clothing_store}_admin_simulator.py`(8091/8090, `POST /_qa/reset` 지원), 자동화: `scripts/qa_tenant_simulation_{setup,run,summarize}.py` |
| [self-service-ai-assistant-e2e-manual-driven-qa-plan.md](qa/self-service-ai-assistant-e2e-manual-driven-qa-plan.md)              | **(이력 보존, v2.0 tenant-simulation 문서로 대체됨)** 2026-08-07 v1.0. 1001 테넌트 × 가상 의류판매 시뮬레이터 27개 케이스 요약형 계획(실행되지 않음)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| [2026-08-28_self_service_tenant_simulation_qa_result.md](reports/2026-08/2026-08-28_self_service_tenant_simulation_qa_result.md) | **테넌트 시뮬레이션 E2E QA 실행 결과(2026-08-28)** — 2라운드 50건 실행. 쓰기 성공률 1/12 → 9/12, 전체 PASS 40% → 68%로 개선. **수정한 근본 원인 5건**(OpenAPI `$ref` 미해석으로 body 스키마 부재 / 식별자 확보 지침 부재 / pre_state 스냅샷 경로 오류로 Undo 구조적 불가 / 경로 파라미터 최상위 전달 / Tool 이름 접두사 누락)과 **잔여 결함 7건**(자연어→ID 매핑 불안정, 매뉴얼 업무 규칙 위반 실행, 플랫폼 온보딩 응답 혼입, RAG 품질·하이브리드 미발동, 승인 세분성 한계 등) 정리                                                                                                                                                                                |
...
| [REFERENCE_TRACEABILITY.md](design/REFERENCE_TRACEABILITY.md)                                                                           | **(2026-08-06, Story 1.47/FR35-H)** 핵심 모듈(intellidecision_policy.py/hybrid_rag.py/knowledge_documents.py 등) ↔ 근거 레퍼런스 ↔ 채택/변형/기각 사유 역참조 인덱스. 기존 리서치 문서의 상세 인용은 그대로 두고 요약·링크만 제공(중복 작성 방지) — 향후 유저 친화적 소개 문서 작성 시 1차 소스                                                                                                                                                                                                                                                                                                                  |
...
| [reports/2026-09/](reports/2026-09/) | 셀프서비스 AI Assistant 소개서·CS 데이터 보강 리포트 |
