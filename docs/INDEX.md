...
| [MCP_VS_CLIENT_CENTRIC_UNIVERSAL_AGENT_MARKET_RESEARCH.md](design/MCP_VS_CLIENT_CENTRIC_UNIVERSAL_AGENT_MARKET_RESEARCH.md) | **(2026-08-05)** "서버 표준화(MCP)" vs "클라이언트가 API 문서만으로 임의 시스템에 적응"(사용자 아이디어) 시장·연구 조사 — OpenAI GPT Actions/ChatGPT Plugins/Zapier NLA→MCP/Composio(상용), Gorilla·GoEx/RestGPT/API-Bank(학계) 6+건 원문·번역·시사점 정리. 결론: 이미 있는 개념(RestGPT·GPT Actions가 가장 근접), 우리 시스템(Story 1.26/1.31)의 차별점은 "런타임 업로드 즉시 반영", 격차는 "실제 실행"(Non-Goal로 미룸) — 코드 변경 없음, 순수 리서치 |
