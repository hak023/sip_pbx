...
### 5.3 잘림(MAX_TOKENS) 근본 원인

**증상**: `finish_reason: "MAX_TOKENS"` → JSON이 `"reason"` 문자열 중간 등에서 끊겨 `Unterminated string` 파싱 에러.

**직접 원인**:  
- API 응답이 `judgment_max_output_tokens`(예: 1024) 한도에 도달해 **출력이 잘림**.  
- 요청(프롬프트/입력) 자체는 정상. 문제는 **출력 토큰 한도**와 **모델이 실제로 쓰는 출력량** 불일치.

**구조적 원인**:
1. **출력 스키마**: `reason`(50자 권장) + `extracted_info[]`(개수·길이 제한 없음).  
   - `extracted_info` 항목이 많거나 각 `text`가 길면 1024 토큰을 쉽게 초과.
2. **설정**: `judgment_max_output_tokens: 1024`는 “reason 짧게 + 항목 소수”일 때만 충분.  
   - 설계 권장은 “1024 이상, 잘림 발생 시 **2048**”.
3. **프롬프트**: “reason 50자 이내”만 있고, **extracted_info 개수·각 text 길이 상한**이 없어 모델이 많은/긴 항목을 쓰다 한도에 걸림.

**대응**:
- **설정**: `judgment_max_output_tokens`를 **2048**로 상향(기본 권장).
- **프롬프트**:  
  - `reason` 50자 **엄수** 재강조.  
  - `extracted_info` **최대 5개**, 각 `text` **200자 이내** 등 상한 명시 → 잘림 확률 감소.
