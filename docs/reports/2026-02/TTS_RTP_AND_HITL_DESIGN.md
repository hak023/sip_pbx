...
- **Notifier**: 동일 오디오 프레임의 재생 길이(바이트→초)를 **응답 1건 단위**(StartFrame ~ EndFrame)로 누적해, EndFrame 시 `last_tts_duration_sec`와 이벤트를 설정한다. StartFrame 수신 시 누적값을 0으로 리셋해 Output과 동일한 구간(이번 응답)을 사용한다.

### 1.2 변수 정의 (로그·동기화 해석용)

| 변수 | 설정 위치 | 의미 |
|------|-----------|------|
| **last_tts_duration_sec** | TTSCompleteNotifier | 해당 응답(StartFrame~EndFrame) 구간에서 TTS가 내보낸 **모든 오디오 프레임**의 재생 길이 합(초). StartFrame 시 리셋, EndFrame 시 기록. `sum(len(audio)/(sample_rate*2*channels))`. "이 응답 음원이 몇 초짜리인가". |
| **bytes_sent** | SIPPBXOutputTransport | 해당 응답(StartFrame~EndFrame) 구간에서 `send_audio_to_caller()`로 **발송 큐에 넣은** PCM 바이트 합. StartFrame 시 리셋. 실제 UDP 전송 완료량이 아님. |
