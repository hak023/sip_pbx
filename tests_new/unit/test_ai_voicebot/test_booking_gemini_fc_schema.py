"""
AI Voicebot Unit Tests - booking_gemini_fc.py의 JSON Schema → glm.Schema 변환 (Story 1.14 근본 원인 수정)

docs/stories/1.14.empty-candidate-string-field-mitigation.story.md 참고
...
"타입 정보 없음"과 "명시적 object 선언"을 똑같이 취급해 빈 OBJECT로 잘못 변환했다. 이로 인해
Gemini function-calling이 실제로는 문자열/불리언 값을 받아야 하는 필드를 "프로퍼티 0개짜리 OBJECT"로
알고 있어, 특히 자유 문자열 값(짧은 불리언/enum 값과 달리)을 채워야 할 때 스키마를 만족시키지 못해
완전히 빈 응답(candidate)을 반환하는 현상으로 이어졌다(결함③).
"""

from google.ai import generativelanguage as glm

from src.ai_voicebot.langgraph.booking_gemini_fc import _json_schema_to_glm_schema


class TestUnconstrainedAnyTypeField:
    """Python `Any` 타입(= JSON Schema에 `type` 키 자체가 없음)의 변환 검증"""

    def test_schema_without_type_key_becomes_string_not_object(self):
        """회귀 방지 핵심 테스트 — 예전에는 OBJECT(속성 0개)로 잘못 변환되었다."""
        schema = {"title": "Value"}
        result = _json_schema_to_glm_schema(schema)
        assert result.type_ == glm.Type.STRING
        assert result.type_ != glm.Type.OBJECT
...
class TestExplicitObjectStillWorks:
    """명시적으로 object로 선언되거나 properties가 있는 경우는 여전히 OBJECT로 변환되어야 함"""
    ...
    def test_properties_without_explicit_type_is_treated_as_object(self):
        """type 키가 없어도 properties가 있으면 여전히 object로 취급(회귀 방지)."""
...
class TestExplicitPrimitiveTypesUnaffected:
    """명시적 타입이 있는 필드는 이번 수정과 무관하게 그대로 동작해야 함"""
    ...
    def test_explicit_boolean_type(self):
        schema = {"type": "boolean"}
        result = _json_schema_to_glm_schema(schema)
        assert result.type_ == glm.Type.BOOLEAN
    ...
    def test_array_type_unaffected(self):
        ...
        result = _json_schema_to_glm_schema(schema)
        assert result.type_ == glm.Type.ARRAY
...
class TestRealUpdateSelfServiceSettingToolSchema:
    ...
    def test_value_field_is_string_not_object(self):
        ...
        glm_tool = _langchain_tools_to_glm_tool([update_self_service_setting_tool])
        decl = glm_tool.function_declarations[0]
        assert decl.parameters.properties["value"].type_ == glm.Type.STRING
    ...
    def test_owner_domain_field_fields_still_string(self):
        ...
        for field in ("owner", "domain", "field", "call_id"):
            assert decl.parameters.properties[field].type_ == glm.Type.STRING
